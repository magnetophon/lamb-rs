...
impl FaustDsp for mydsp {
	...
	fn compute(&mut self, count: i32, inputs: &[&[Self::T]], outputs: &mut[&mut[Self::T]]) {
		...
		for (((input0, input1), output0), output1) in zipped_iterators {
			...
			let mut fTemp21: F64 = F64::powf(1e+01, -(0.05 * F64::max(0.0, if (iTemp19 == 0) as i32 != 0 {0.0} else {if (iTemp19 == 1) as i32 != 0 {fSlow12 * mydsp_faustpower2_f(fSlow7 + fTemp20)} else {fTemp20}})));
			...
			let mut fTemp40: F64 = F64::min(fTemp33, F64::powf(1e+01, -(fSlow18 * F64::max(0.0, if (iTemp38 == 0) as i32 != 0 {0.0} else {if (iTemp38 == 1) as i32 != 0 {fSlow12 * mydsp_faustpower2_f(fSlow7 + fTemp39)} else {fTemp39}}))));
			...
			let mut fTemp73: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp72, 3)) as usize] };
			let mut fTemp74: F64 = unsafe { ftbl0mydspSIG0[iTemp72 as usize] };
			let mut fTemp75: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp72, 1)) as usize] } - fTemp74;
			let mut fTemp76: F64 = fTemp69 - (iTemp70) as F64;
			let mut fTemp77: F64 = fTemp74 + fTemp76 * fTemp75 + 0.5 * (fTemp73 - (fTemp74 + fTemp76 * (fTemp75 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp72, 4)) as usize] } - fTemp73))));
			...
			let mut fTemp86: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp85, 3)) as usize] };
			let mut fTemp87: F64 = unsafe { ftbl0mydspSIG0[iTemp85 as usize] };
			let mut fTemp88: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp85, 1)) as usize] } - fTemp87;
			...
			let mut fTemp92: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp91, 3), 196607))) as usize] };
			let mut fTemp93: F64 = unsafe { ftbl0mydspSIG0[iTemp91 as usize] };
			let mut fTemp94: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp91, 1), 196607))) as usize] } - fTemp93;
			...
			let mut fTemp101: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp100, 3), 196607))) as usize] };
			let mut fTemp102: F64 = unsafe { ftbl0mydspSIG0[iTemp100 as usize] };
			let mut fTemp103: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp100, 1), 196607))) as usize] } - fTemp102;
			...
			let mut fTemp108: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp107, 3), 196607))) as usize] };
			let mut fTemp109: F64 = unsafe { ftbl0mydspSIG0[iTemp107 as usize] };
			let mut fTemp110: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp107, 1), 196607))) as usize] } - fTemp109;
			...
			let mut fTemp115: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp114, 3)) as usize] };
			let mut fTemp116: F64 = unsafe { ftbl0mydspSIG0[iTemp114 as usize] };
			let mut fTemp117: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp114, 1)) as usize] } - fTemp116;
			...
			let mut fTemp121: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp120, 3), 196607))) as usize] };
			let mut fTemp122: F64 = unsafe { ftbl0mydspSIG0[iTemp120 as usize] };
			let mut fTemp123: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp120, 1), 196607))) as usize] } - fTemp122;
			...
			let mut fTemp134: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp133, 3), 196607))) as usize] };
			let mut fTemp135: F64 = unsafe { ftbl0mydspSIG0[iTemp133 as usize] };
			let mut fTemp136: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp133, 1), 196607))) as usize] } - fTemp135;
			let mut fTemp137: F64 = (if iTemp62 != 0 {fTemp135 + fTemp76 * fTemp136 + (fTemp131 - (iTemp132) as F64) * (fTemp134 - (fTemp135 + fTemp76 * (fTemp136 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp133, 4), 196607))) as usize] } - fTemp134))))} else {1.0 - (fTemp129 + fTemp76 * fTemp130 + (fTemp125 - (iTemp126) as F64) * (fTemp128 - (fTemp129 + fTemp76 * (fTemp130 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp127, 4)) as usize] } - fTemp128)))))} - if iTemp62 != 0 {fTemp122 + fTemp76 * fTemp123 + (fTemp118 - (iTemp119) as F64) * (fTemp121 - (fTemp122 + fTemp76 * (fTemp123 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp120, 4), 196607))) as usize] } - fTemp121))))} else {1.0 - (fTemp116 + fTemp76 * fTemp117 + (fTemp112 - (iTemp113) as F64) * (fTemp115 - (fTemp116 + fTemp76 * (fTemp117 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp114, 4), 196607))) as usize] } - fTemp115)))))}) * self.fVec27[1] / (fTemp61 * (1.0 - if iTemp62 != 0 {fTemp109 + fTemp104 * fTemp110 + (fTemp105 - (iTemp106) as F64) * (fTemp108 - (fTemp109 + fTemp104 * (fTemp110 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp107, 4), 196607))) as usize] } - fTemp108))))} else {1.0 - (fTemp102 + fTemp104 * fTemp103 + (fTemp98 - (iTemp99) as F64) * (fTemp101 - (fTemp102 + fTemp104 * (fTemp103 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp100, 4), 196607))) as usize] } - fTemp101)))))}));
			let mut iTemp138: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp93 + fTemp76 * fTemp94 + (fTemp89 - (iTemp90) as F64) * (fTemp92 - (fTemp93 + fTemp76 * (fTemp94 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp91, 4), 196607))) as usize] } - fTemp92))))} else {1.0 - (fTemp87 + fTemp76 * fTemp88 + (fTemp83 - (iTemp84) as F64) * (fTemp86 - (fTemp87 + fTemp76 * (fTemp88 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp85, 4)) as usize] } - fTemp86)))))} - fTemp78) / (1.0 - fTemp78))) as i32;
			...
			let mut fTemp146: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp145, 3)) as usize] };
			let mut fTemp147: F64 = unsafe { ftbl0mydspSIG0[iTemp145 as usize] };
			let mut fTemp148: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp145, 1)) as usize] } - fTemp147;
			...
			let mut fTemp166: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp165, 3), 196607))) as usize] };
			let mut fTemp167: F64 = unsafe { ftbl0mydspSIG0[iTemp165 as usize] };
			let mut fTemp168: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp165, 1), 196607))) as usize] } - fTemp167;
			...
			let mut fTemp177: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp176, 3)) as usize] };
			let mut fTemp178: F64 = unsafe { ftbl0mydspSIG0[iTemp176 as usize] };
			let mut fTemp179: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp176, 1)) as usize] } - fTemp178;
			...
			let mut fTemp186: F64 = if iTemp62 != 0 {fTemp184 + fTemp76 * fTemp185 + (fTemp180 - (iTemp181) as F64) * (fTemp183 - (fTemp184 + fTemp76 * (fTemp185 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp182, 4)) as usize] } - fTemp183))))} else {1.0 - (fTemp178 + fTemp76 * fTemp179 + (fTemp174 - (iTemp175) as F64) * (fTemp177 - (fTemp178 + fTemp76 * (fTemp179 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp176, 4)) as usize] } - fTemp177)))))};
			...
			let mut fTemp197: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp196, 3), 196607))) as usize] };
			let mut fTemp198: F64 = unsafe { ftbl0mydspSIG0[iTemp196 as usize] };
			let mut fTemp199: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp196, 1), 196607))) as usize] } - fTemp198;
			let mut iTemp200: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp198 + fTemp76 * fTemp199 + (fTemp194 - (iTemp195) as F64) * (fTemp197 - (fTemp198 + fTemp76 * (fTemp199 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp196, 4), 196607))) as usize] } - fTemp197))))} else {1.0 - (fTemp192 + fTemp76 * fTemp193 + (fTemp188 - (iTemp189) as F64) * (fTemp191 - (fTemp192 + fTemp76 * (fTemp193 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp190, 4)) as usize] } - fTemp191)))))} - fTemp186) / (1.0 - fTemp186))) as i32;
			...
			let mut fTemp208: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp207, 3)) as usize] };
			let mut fTemp209: F64 = unsafe { ftbl0mydspSIG0[iTemp207 as usize] };
			let mut fTemp210: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp207, 1)) as usize] } - fTemp209;
			...
			let mut fTemp217: F64 = if iTemp62 != 0 {fTemp215 + fTemp76 * fTemp216 + (fTemp211 - (iTemp212) as F64) * (fTemp214 - (fTemp215 + fTemp76 * (fTemp216 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp213, 4)) as usize] } - fTemp214))))} else {1.0 - (fTemp209 + fTemp76 * fTemp210 + (fTemp205 - (iTemp206) as F64) * (fTemp208 - (fTemp209 + fTemp76 * (fTemp210 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp207, 4)) as usize] } - fTemp208)))))};
			...
			let mut fTemp228: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp227, 3), 196607))) as usize] };
			let mut fTemp229: F64 = unsafe { ftbl0mydspSIG0[iTemp227 as usize] };
			let mut fTemp230: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp227, 1), 196607))) as usize] } - fTemp229;
			let mut iTemp231: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp229 + fTemp76 * fTemp230 + (fTemp225 - (iTemp226) as F64) * (fTemp228 - (fTemp229 + fTemp76 * (fTemp230 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp227, 4), 196607))) as usize] } - fTemp228))))} else {1.0 - (fTemp223 + fTemp76 * fTemp224 + (fTemp219 - (iTemp220) as F64) * (fTemp222 - (fTemp223 + fTemp76 * (fTemp224 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp221, 4)) as usize] } - fTemp222)))))} - fTemp217) / (1.0 - fTemp217))) as i32;
			...
			let mut fTemp239: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp238, 3)) as usize] };
			let mut fTemp240: F64 = unsafe { ftbl0mydspSIG0[iTemp238 as usize] };
			let mut fTemp241: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp238, 1)) as usize] } - fTemp240;
			...
			let mut fTemp248: F64 = if iTemp62 != 0 {fTemp246 + fTemp76 * fTemp247 + (fTemp242 - (iTemp243) as F64) * (fTemp245 - (fTemp246 + fTemp76 * (fTemp247 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp244, 4)) as usize] } - fTemp245))))} else {1.0 - (fTemp240 + fTemp76 * fTemp241 + (fTemp236 - (iTemp237) as F64) * (fTemp239 - (fTemp240 + fTemp76 * (fTemp241 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp238, 4)) as usize] } - fTemp239)))))};
			...
			let mut fTemp259: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp258, 3), 196607))) as usize] };
			let mut fTemp260: F64 = unsafe { ftbl0mydspSIG0[iTemp258 as usize] };
			let mut fTemp261: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp258, 1), 196607))) as usize] } - fTemp260;
			let mut iTemp262: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp260 + fTemp76 * fTemp261 + (fTemp256 - (iTemp257) as F64) * (fTemp259 - (fTemp260 + fTemp76 * (fTemp261 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp258, 4), 196607))) as usize] } - fTemp259))))} else {1.0 - (fTemp254 + fTemp76 * fTemp255 + (fTemp250 - (iTemp251) as F64) * (fTemp253 - (fTemp254 + fTemp76 * (fTemp255 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp252, 4)) as usize] } - fTemp253)))))} - fTemp248) / (1.0 - fTemp248))) as i32;
			...
			let mut fTemp270: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp269, 3)) as usize] };
			let mut fTemp271: F64 = unsafe { ftbl0mydspSIG0[iTemp269 as usize] };
			let mut fTemp272: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp269, 1)) as usize] } - fTemp271;
			...
			let mut fTemp279: F64 = if iTemp62 != 0 {fTemp277 + fTemp76 * fTemp278 + (fTemp273 - (iTemp274) as F64) * (fTemp276 - (fTemp277 + fTemp76 * (fTemp278 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp275, 4)) as usize] } - fTemp276))))} else {1.0 - (fTemp271 + fTemp76 * fTemp272 + (fTemp267 - (iTemp268) as F64) * (fTemp270 - (fTemp271 + fTemp76 * (fTemp272 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp269, 4)) as usize] } - fTemp270)))))};
			...
			let mut fTemp290: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp289, 3), 196607))) as usize] };
			let mut fTemp291: F64 = unsafe { ftbl0mydspSIG0[iTemp289 as usize] };
			let mut fTemp292: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp289, 1), 196607))) as usize] } - fTemp291;
			let mut iTemp293: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp291 + fTemp76 * fTemp292 + (fTemp287 - (iTemp288) as F64) * (fTemp290 - (fTemp291 + fTemp76 * (fTemp292 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp289, 4), 196607))) as usize] } - fTemp290))))} else {1.0 - (fTemp285 + fTemp76 * fTemp286 + (fTemp281 - (iTemp282) as F64) * (fTemp284 - (fTemp285 + fTemp76 * (fTemp286 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp283, 4)) as usize] } - fTemp284)))))} - fTemp279) / (1.0 - fTemp279))) as i32;
			...
			let mut fTemp301: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp300, 3)) as usize] };
			let mut fTemp302: F64 = unsafe { ftbl0mydspSIG0[iTemp300 as usize] };
			let mut fTemp303: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp300, 1)) as usize] } - fTemp302;
			...
			let mut fTemp310: F64 = if iTemp62 != 0 {fTemp308 + fTemp76 * fTemp309 + (fTemp304 - (iTemp305) as F64) * (fTemp307 - (fTemp308 + fTemp76 * (fTemp309 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp306, 4)) as usize] } - fTemp307))))} else {1.0 - (fTemp302 + fTemp76 * fTemp303 + (fTemp298 - (iTemp299) as F64) * (fTemp301 - (fTemp302 + fTemp76 * (fTemp303 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp300, 4)) as usize] } - fTemp301)))))};
			...
			let mut fTemp321: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp320, 3), 196607))) as usize] };
			let mut fTemp322: F64 = unsafe { ftbl0mydspSIG0[iTemp320 as usize] };
			let mut fTemp323: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp320, 1), 196607))) as usize] } - fTemp322;
			...
			let mut fTemp332: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp331, 3)) as usize] };
			let mut fTemp333: F64 = unsafe { ftbl0mydspSIG0[iTemp331 as usize] };
			let mut fTemp334: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp331, 1)) as usize] } - fTemp333;
			...
			let mut fTemp341: F64 = if iTemp62 != 0 {fTemp339 + fTemp76 * fTemp340 + (fTemp335 - (iTemp336) as F64) * (fTemp338 - (fTemp339 + fTemp76 * (fTemp340 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp337, 4)) as usize] } - fTemp338))))} else {1.0 - (fTemp333 + fTemp76 * fTemp334 + (fTemp329 - (iTemp330) as F64) * (fTemp332 - (fTemp333 + fTemp76 * (fTemp334 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp331, 4)) as usize] } - fTemp332)))))};
			...
			let mut fTemp352: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp351, 3), 196607))) as usize] };
			let mut fTemp353: F64 = unsafe { ftbl0mydspSIG0[iTemp351 as usize] };
			let mut fTemp354: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp351, 1), 196607))) as usize] } - fTemp353;
			let mut iTemp355: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp353 + fTemp76 * fTemp354 + (fTemp349 - (iTemp350) as F64) * (fTemp352 - (fTemp353 + fTemp76 * (fTemp354 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp351, 4), 196607))) as usize] } - fTemp352))))} else {1.0 - (fTemp347 + fTemp76 * fTemp348 + (fTemp343 - (iTemp344) as F64) * (fTemp346 - (fTemp347 + fTemp76 * (fTemp348 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp345, 4)) as usize] } - fTemp346)))))} - fTemp341) / (1.0 - fTemp341))) as i32;
			...
			let mut fTemp363: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp362, 3)) as usize] };
			let mut fTemp364: F64 = unsafe { ftbl0mydspSIG0[iTemp362 as usize] };
			let mut fTemp365: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp362, 1)) as usize] } - fTemp364;
			...
			let mut fTemp383: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp382, 3), 196607))) as usize] };
			let mut fTemp384: F64 = unsafe { ftbl0mydspSIG0[iTemp382 as usize] };
			let mut fTemp385: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp382, 1), 196607))) as usize] } - fTemp384;
			let mut iTemp386: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp384 + fTemp76 * fTemp385 + (fTemp380 - (iTemp381) as F64) * (fTemp383 - (fTemp384 + fTemp76 * (fTemp385 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp382, 4), 196607))) as usize] } - fTemp383))))} else {1.0 - (fTemp378 + fTemp76 * fTemp379 + (fTemp374 - (iTemp375) as F64) * (fTemp377 - (fTemp378 + fTemp76 * (fTemp379 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp376, 4)) as usize] } - fTemp377)))))} - fTemp372) / (1.0 - fTemp372))) as i32;
			...
			let mut fTemp394: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp393, 3)) as usize] };
			let mut fTemp395: F64 = unsafe { ftbl0mydspSIG0[iTemp393 as usize] };
			let mut fTemp396: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp393, 1)) as usize] } - fTemp395;
			...
			let mut fTemp403: F64 = if iTemp62 != 0 {fTemp401 + fTemp76 * fTemp402 + (fTemp397 - (iTemp398) as F64) * (fTemp400 - (fTemp401 + fTemp76 * (fTemp402 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp399, 4)) as usize] } - fTemp400))))} else {1.0 - (fTemp395 + fTemp76 * fTemp396 + (fTemp391 - (iTemp392) as F64) * (fTemp394 - (fTemp395 + fTemp76 * (fTemp396 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp393, 4)) as usize] } - fTemp394)))))};
			...
			let mut fTemp414: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp413, 3), 196607))) as usize] };
			let mut fTemp415: F64 = unsafe { ftbl0mydspSIG0[iTemp413 as usize] };
			let mut fTemp416: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp413, 1), 196607))) as usize] } - fTemp415;
			let mut iTemp417: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp415 + fTemp76 * fTemp416 + (fTemp411 - (iTemp412) as F64) * (fTemp414 - (fTemp415 + fTemp76 * (fTemp416 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp413, 4), 196607))) as usize] } - fTemp414))))} else {1.0 - (fTemp409 + fTemp76 * fTemp410 + (fTemp405 - (iTemp406) as F64) * (fTemp408 - (fTemp409 + fTemp76 * (fTemp410 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp407, 4)) as usize] } - fTemp408)))))} - fTemp403) / (1.0 - fTemp403))) as i32;
			...
			let mut fTemp425: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp424, 3)) as usize] };
			let mut fTemp426: F64 = unsafe { ftbl0mydspSIG0[iTemp424 as usize] };
			let mut fTemp427: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp424, 1)) as usize] } - fTemp426;
			...
			let mut fTemp434: F64 = if iTemp62 != 0 {fTemp432 + fTemp76 * fTemp433 + (fTemp428 - (iTemp429) as F64) * (fTemp431 - (fTemp432 + fTemp76 * (fTemp433 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp430, 4)) as usize] } - fTemp431))))} else {1.0 - (fTemp426 + fTemp76 * fTemp427 + (fTemp422 - (iTemp423) as F64) * (fTemp425 - (fTemp426 + fTemp76 * (fTemp427 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp424, 4)) as usize] } - fTemp425)))))};
			...
			let mut fTemp445: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp444, 3), 196607))) as usize] };
			let mut fTemp446: F64 = unsafe { ftbl0mydspSIG0[iTemp444 as usize] };
			let mut fTemp447: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp444, 1), 196607))) as usize] } - fTemp446;
			let mut iTemp448: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp446 + fTemp76 * fTemp447 + (fTemp442 - (iTemp443) as F64) * (fTemp445 - (fTemp446 + fTemp76 * (fTemp447 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp444, 4), 196607))) as usize] } - fTemp445))))} else {1.0 - (fTemp440 + fTemp76 * fTemp441 + (fTemp436 - (iTemp437) as F64) * (fTemp439 - (fTemp440 + fTemp76 * (fTemp441 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp438, 4)) as usize] } - fTemp439)))))} - fTemp434) / (1.0 - fTemp434))) as i32;
			...
			let mut fTemp456: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp455, 3)) as usize] };
			let mut fTemp457: F64 = unsafe { ftbl0mydspSIG0[iTemp455 as usize] };
			let mut fTemp458: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp455, 1)) as usize] } - fTemp457;
			...
			let mut fTemp465: F64 = if iTemp62 != 0 {fTemp463 + fTemp76 * fTemp464 + (fTemp459 - (iTemp460) as F64) * (fTemp462 - (fTemp463 + fTemp76 * (fTemp464 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp461, 4)) as usize] } - fTemp462))))} else {1.0 - (fTemp457 + fTemp76 * fTemp458 + (fTemp453 - (iTemp454) as F64) * (fTemp456 - (fTemp457 + fTemp76 * (fTemp458 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp455, 4)) as usize] } - fTemp456)))))};
			...
			let mut fTemp476: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp475, 3), 196607))) as usize] };
			let mut fTemp477: F64 = unsafe { ftbl0mydspSIG0[iTemp475 as usize] };
			let mut fTemp478: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp475, 1), 196607))) as usize] } - fTemp477;
			let mut iTemp479: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp477 + fTemp76 * fTemp478 + (fTemp473 - (iTemp474) as F64) * (fTemp476 - (fTemp477 + fTemp76 * (fTemp478 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp475, 4), 196607))) as usize] } - fTemp476))))} else {1.0 - (fTemp471 + fTemp76 * fTemp472 + (fTemp467 - (iTemp468) as F64) * (fTemp470 - (fTemp471 + fTemp76 * (fTemp472 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp469, 4)) as usize] } - fTemp470)))))} - fTemp465) / (1.0 - fTemp465))) as i32;
			...
			let mut fTemp487: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp486, 3)) as usize] };
			let mut fTemp488: F64 = unsafe { ftbl0mydspSIG0[iTemp486 as usize] };
			let mut fTemp489: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp486, 1)) as usize] } - fTemp488;
			...
			let mut fTemp507: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp506, 3), 196607))) as usize] };
			let mut fTemp508: F64 = unsafe { ftbl0mydspSIG0[iTemp506 as usize] };
			let mut fTemp509: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp506, 1), 196607))) as usize] } - fTemp508;
			let mut iTemp510: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp508 + fTemp76 * fTemp509 + (fTemp504 - (iTemp505) as F64) * (fTemp507 - (fTemp508 + fTemp76 * (fTemp509 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp506, 4), 196607))) as usize] } - fTemp507))))} else {1.0 - (fTemp502 + fTemp76 * fTemp503 + (fTemp498 - (iTemp499) as F64) * (fTemp501 - (fTemp502 + fTemp76 * (fTemp503 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp500, 4)) as usize] } - fTemp501)))))} - fTemp496) / (1.0 - fTemp496))) as i32;
			...
			let mut fTemp518: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp517, 3)) as usize] };
			let mut fTemp519: F64 = unsafe { ftbl0mydspSIG0[iTemp517 as usize] };
			let mut fTemp520: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp517, 1)) as usize] } - fTemp519;
			...
			let mut fTemp527: F64 = if iTemp62 != 0 {fTemp525 + fTemp76 * fTemp526 + (fTemp521 - (iTemp522) as F64) * (fTemp524 - (fTemp525 + fTemp76 * (fTemp526 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp523, 4)) as usize] } - fTemp524))))} else {1.0 - (fTemp519 + fTemp76 * fTemp520 + (fTemp515 - (iTemp516) as F64) * (fTemp518 - (fTemp519 + fTemp76 * (fTemp520 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp517, 4)) as usize] } - fTemp518)))))};
			...
			let mut fTemp538: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp537, 3), 196607))) as usize] };
			let mut fTemp539: F64 = unsafe { ftbl0mydspSIG0[iTemp537 as usize] };
			let mut fTemp540: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp537, 1), 196607))) as usize] } - fTemp539;
			let mut iTemp541: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp539 + fTemp76 * fTemp540 + (fTemp535 - (iTemp536) as F64) * (fTemp538 - (fTemp539 + fTemp76 * (fTemp540 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp537, 4), 196607))) as usize] } - fTemp538))))} else {1.0 - (fTemp533 + fTemp76 * fTemp534 + (fTemp529 - (iTemp530) as F64) * (fTemp532 - (fTemp533 + fTemp76 * (fTemp534 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp531, 4)) as usize] } - fTemp532)))))} - fTemp527) / (1.0 - fTemp527))) as i32;
			...
			let mut fTemp549: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp548, 3)) as usize] };
			let mut fTemp550: F64 = unsafe { ftbl0mydspSIG0[iTemp548 as usize] };
			let mut fTemp551: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp548, 1)) as usize] } - fTemp550;
			...
			let mut fTemp558: F64 = if iTemp62 != 0 {fTemp556 + fTemp76 * fTemp557 + (fTemp552 - (iTemp553) as F64) * (fTemp555 - (fTemp556 + fTemp76 * (fTemp557 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp554, 4)) as usize] } - fTemp555))))} else {1.0 - (fTemp550 + fTemp76 * fTemp551 + (fTemp546 - (iTemp547) as F64) * (fTemp549 - (fTemp550 + fTemp76 * (fTemp551 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp548, 4)) as usize] } - fTemp549)))))};
			...
			let mut fTemp569: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp568, 3), 196607))) as usize] };
			let mut fTemp570: F64 = unsafe { ftbl0mydspSIG0[iTemp568 as usize] };
			let mut fTemp571: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp568, 1), 196607))) as usize] } - fTemp570;
			let mut iTemp572: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp570 + fTemp76 * fTemp571 + (fTemp566 - (iTemp567) as F64) * (fTemp569 - (fTemp570 + fTemp76 * (fTemp571 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp568, 4), 196607))) as usize] } - fTemp569))))} else {1.0 - (fTemp564 + fTemp76 * fTemp565 + (fTemp560 - (iTemp561) as F64) * (fTemp563 - (fTemp564 + fTemp76 * (fTemp565 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp562, 4)) as usize] } - fTemp563)))))} - fTemp558) / (1.0 - fTemp558))) as i32;
			...
			let mut fTemp580: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp579, 3)) as usize] };
			let mut fTemp581: F64 = unsafe { ftbl0mydspSIG0[iTemp579 as usize] };
			let mut fTemp582: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp579, 1)) as usize] } - fTemp581;
			...
			let mut fTemp594: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp593, 3)) as usize] };
			let mut fTemp595: F64 = unsafe { ftbl0mydspSIG0[iTemp593 as usize] };
			let mut fTemp596: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp593, 1)) as usize] } - fTemp595;
			...
			let mut fTemp600: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp599, 3), 196607))) as usize] };
			let mut fTemp601: F64 = unsafe { ftbl0mydspSIG0[iTemp599 as usize] };
			let mut fTemp602: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp599, 1), 196607))) as usize] } - fTemp601;
			let mut iTemp603: i32 = (fTemp137 > ((if iTemp62 != 0 {fTemp601 + fTemp76 * fTemp602 + (fTemp597 - (iTemp598) as F64) * (fTemp600 - (fTemp601 + fTemp76 * (fTemp602 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp599, 4), 196607))) as usize] } - fTemp600))))} else {1.0 - (fTemp595 + fTemp76 * fTemp596 + (fTemp591 - (iTemp592) as F64) * (fTemp594 - (fTemp595 + fTemp76 * (fTemp596 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp593, 4)) as usize] } - fTemp594)))))} - fTemp589) / (1.0 - fTemp589))) as i32;
			...
			let mut fTemp608: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp607, 3)) as usize] };
			let mut fTemp609: F64 = unsafe { ftbl0mydspSIG0[iTemp607 as usize] };
			let mut fTemp610: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp607, 1)) as usize] } - fTemp609;
			...
			let mut fTemp617: F64 = if iTemp62 != 0 {fTemp615 + fTemp76 * fTemp616 + (fTemp611 - (iTemp612) as F64) * (fTemp614 - (fTemp615 + fTemp76 * (fTemp616 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp613, 4), 196607))) as usize] } - fTemp614))))} else {1.0 - (fTemp609 + fTemp76 * fTemp610 + (fTemp605 - (iTemp606) as F64) * (fTemp608 - (fTemp609 + fTemp76 * (fTemp610 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp607, 4), 196607))) as usize] } - fTemp608)))))};
			...
			let mut fTemp622: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp621, 3)) as usize] };
			let mut fTemp623: F64 = unsafe { ftbl0mydspSIG0[iTemp621 as usize] };
			let mut fTemp624: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp621, 1)) as usize] } - fTemp623;
			...
			let mut fTemp628: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp627, 3), 196607))) as usize] };
			let mut fTemp629: F64 = unsafe { ftbl0mydspSIG0[iTemp627 as usize] };
			let mut fTemp630: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp627, 1), 196607))) as usize] } - fTemp629;
			let mut fTemp631: F64 = fTemp4 + if ((0.001 * fTemp80) == 0.0) as i32 != 0 {fTemp61} else {fTemp61 * (if iTemp62 != 0 {fTemp629 + fTemp76 * fTemp630 + (fTemp625 - (iTemp626) as F64) * (fTemp628 - (fTemp629 + fTemp76 * (fTemp630 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp627, 4), 196607))) as usize] } - fTemp628))))} else {1.0 - (fTemp623 + fTemp76 * fTemp624 + (fTemp619 - (iTemp620) as F64) * (fTemp622 - (fTemp623 + fTemp76 * (fTemp624 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp621, 4)) as usize] } - fTemp622)))))} - fTemp617) / (1.0 - fTemp617)};
			...
			*output0 = 0.5 * self.fVec0[((i32::wrapping_sub(self.IOTA0, iSlow71)) & 32767) as usize] * fTemp2 + self.fRec14[0] * self.fVec1[((i32::wrapping_sub(self.IOTA0, iSlow71)) & 32767) as usize] * fTemp632 * fTemp3;
			...
			let mut fTemp637: F64 = F64::min(fTemp33, F64::powf(1e+01, -(fSlow18 * F64::max(0.0, if (iTemp635 == 0) as i32 != 0 {0.0} else {if (iTemp635 == 1) as i32 != 0 {fSlow12 * mydsp_faustpower2_f(fSlow7 + fTemp636)} else {fTemp636}}))));
			...
			let mut fTemp665: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp664, 3)) as usize] };
			let mut fTemp666: F64 = unsafe { ftbl0mydspSIG0[iTemp664 as usize] };
			let mut fTemp667: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp664, 1)) as usize] } - fTemp666;
			let mut fTemp668: F64 = fTemp661 - (iTemp662) as F64;
			let mut fTemp669: F64 = fTemp666 + fTemp668 * fTemp667 + 0.5 * (fTemp665 - (fTemp666 + fTemp668 * (fTemp667 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp664, 4)) as usize] } - fTemp665))));
			...
			let mut fTemp678: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp677, 3)) as usize] };
			let mut fTemp679: F64 = unsafe { ftbl0mydspSIG0[iTemp677 as usize] };
			let mut fTemp680: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp677, 1)) as usize] } - fTemp679;
			...
			let mut fTemp684: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp683, 3), 196607))) as usize] };
			let mut fTemp685: F64 = unsafe { ftbl0mydspSIG0[iTemp683 as usize] };
			let mut fTemp686: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp683, 1), 196607))) as usize] } - fTemp685;
			...
			let mut fTemp693: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp692, 3), 196607))) as usize] };
			let mut fTemp694: F64 = unsafe { ftbl0mydspSIG0[iTemp692 as usize] };
			let mut fTemp695: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp692, 1), 196607))) as usize] } - fTemp694;
			...
			let mut fTemp700: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp699, 3), 196607))) as usize] };
			let mut fTemp701: F64 = unsafe { ftbl0mydspSIG0[iTemp699 as usize] };
			let mut fTemp702: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp699, 1), 196607))) as usize] } - fTemp701;
			...
			let mut fTemp707: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp706, 3)) as usize] };
			let mut fTemp708: F64 = unsafe { ftbl0mydspSIG0[iTemp706 as usize] };
			let mut fTemp709: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp706, 1)) as usize] } - fTemp708;
			...
			let mut fTemp713: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp712, 3), 196607))) as usize] };
			let mut fTemp714: F64 = unsafe { ftbl0mydspSIG0[iTemp712 as usize] };
			let mut fTemp715: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp712, 1), 196607))) as usize] } - fTemp714;
			...
			let mut fTemp738: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp737, 3)) as usize] };
			let mut fTemp739: F64 = unsafe { ftbl0mydspSIG0[iTemp737 as usize] };
			let mut fTemp740: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp737, 1)) as usize] } - fTemp739;
			...
			let mut fTemp752: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp751, 3)) as usize] };
			let mut fTemp753: F64 = unsafe { ftbl0mydspSIG0[iTemp751 as usize] };
			let mut fTemp754: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp751, 1)) as usize] } - fTemp753;
			...
			let mut fTemp758: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp757, 3), 196607))) as usize] };
			let mut fTemp759: F64 = unsafe { ftbl0mydspSIG0[iTemp757 as usize] };
			let mut fTemp760: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp757, 1), 196607))) as usize] } - fTemp759;
			let mut iTemp761: i32 = (fTemp729 > ((if iTemp659 != 0 {fTemp759 + fTemp668 * fTemp760 + (fTemp755 - (iTemp756) as F64) * (fTemp758 - (fTemp759 + fTemp668 * (fTemp760 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp757, 4), 196607))) as usize] } - fTemp758))))} else {1.0 - (fTemp753 + fTemp668 * fTemp754 + (fTemp749 - (iTemp750) as F64) * (fTemp752 - (fTemp753 + fTemp668 * (fTemp754 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp751, 4)) as usize] } - fTemp752)))))} - fTemp747) / (1.0 - fTemp747))) as i32;
			...
			let mut fTemp769: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp768, 3)) as usize] };
			let mut fTemp770: F64 = unsafe { ftbl0mydspSIG0[iTemp768 as usize] };
			let mut fTemp771: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp768, 1)) as usize] } - fTemp770;
			...
			let mut fTemp783: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp782, 3)) as usize] };
			let mut fTemp784: F64 = unsafe { ftbl0mydspSIG0[iTemp782 as usize] };
			let mut fTemp785: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp782, 1)) as usize] } - fTemp784;
			...
			let mut fTemp789: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp788, 3), 196607))) as usize] };
			let mut fTemp790: F64 = unsafe { ftbl0mydspSIG0[iTemp788 as usize] };
			let mut fTemp791: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp788, 1), 196607))) as usize] } - fTemp790;
			let mut iTemp792: i32 = (fTemp729 > ((if iTemp659 != 0 {fTemp790 + fTemp668 * fTemp791 + (fTemp786 - (iTemp787) as F64) * (fTemp789 - (fTemp790 + fTemp668 * (fTemp791 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp788, 4), 196607))) as usize] } - fTemp789))))} else {1.0 - (fTemp784 + fTemp668 * fTemp785 + (fTemp780 - (iTemp781) as F64) * (fTemp783 - (fTemp784 + fTemp668 * (fTemp785 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp782, 4)) as usize] } - fTemp783)))))} - fTemp778) / (1.0 - fTemp778))) as i32;
			...
			let mut fTemp800: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp799, 3)) as usize] };
			let mut fTemp801: F64 = unsafe { ftbl0mydspSIG0[iTemp799 as usize] };
			let mut fTemp802: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp799, 1)) as usize] } - fTemp801;
			...
			let mut fTemp809: F64 = if iTemp659 != 0 {fTemp807 + fTemp668 * fTemp808 + (fTemp803 - (iTemp804) as F64) * (fTemp806 - (fTemp807 + fTemp668 * (fTemp808 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp805, 4)) as usize] } - fTemp806))))} else {1.0 - (fTemp801 + fTemp668 * fTemp802 + (fTemp797 - (iTemp798) as F64) * (fTemp800 - (fTemp801 + fTemp668 * (fTemp802 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp799, 4)) as usize] } - fTemp800)))))};
			...
			let mut fTemp814: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp813, 3)) as usize] };
			let mut fTemp815: F64 = unsafe { ftbl0mydspSIG0[iTemp813 as usize] };
			let mut fTemp816: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp813, 1)) as usize] } - fTemp815;
			...
			let mut fTemp820: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp819, 3), 196607))) as usize] };
			let mut fTemp821: F64 = unsafe { ftbl0mydspSIG0[iTemp819 as usize] };
			let mut fTemp822: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp819, 1), 196607))) as usize] } - fTemp821;
			...
			let mut fTemp831: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp830, 3)) as usize] };
			let mut fTemp832: F64 = unsafe { ftbl0mydspSIG0[iTemp830 as usize] };
			let mut fTemp833: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp830, 1)) as usize] } - fTemp832;
			...
			let mut fTemp840: F64 = if iTemp659 != 0 {fTemp838 + fTemp668 * fTemp839 + (fTemp834 - (iTemp835) as F64) * (fTemp837 - (fTemp838 + fTemp668 * (fTemp839 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp836, 4)) as usize] } - fTemp837))))} else {1.0 - (fTemp832 + fTemp668 * fTemp833 + (fTemp828 - (iTemp829) as F64) * (fTemp831 - (fTemp832 + fTemp668 * (fTemp833 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp830, 4)) as usize] } - fTemp831)))))};
			...
			let mut fTemp845: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp844, 3)) as usize] };
			let mut fTemp846: F64 = unsafe { ftbl0mydspSIG0[iTemp844 as usize] };
			let mut fTemp847: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp844, 1)) as usize] } - fTemp846;
			...
			let mut fTemp851: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp850, 3), 196607))) as usize] };
			let mut fTemp852: F64 = unsafe { ftbl0mydspSIG0[iTemp850 as usize] };
			let mut fTemp853: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp850, 1), 196607))) as usize] } - fTemp852;
			let mut iTemp854: i32 = (fTemp729 > ((if iTemp659 != 0 {fTemp852 + fTemp668 * fTemp853 + (fTemp848 - (iTemp849) as F64) * (fTemp851 - (fTemp852 + fTemp668 * (fTemp853 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp850, 4), 196607))) as usize] } - fTemp851))))} else {1.0 - (fTemp846 + fTemp668 * fTemp847 + (fTemp842 - (iTemp843) as F64) * (fTemp845 - (fTemp846 + fTemp668 * (fTemp847 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp844, 4)) as usize] } - fTemp845)))))} - fTemp840) / (1.0 - fTemp840))) as i32;
			...
			let mut fTemp862: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp861, 3)) as usize] };
			let mut fTemp863: F64 = unsafe { ftbl0mydspSIG0[iTemp861 as usize] };
			let mut fTemp864: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp861, 1)) as usize] } - fTemp863;
			...
			let mut fTemp871: F64 = if iTemp659 != 0 {fTemp869 + fTemp668 * fTemp870 + (fTemp865 - (iTemp866) as F64) * (fTemp868 - (fTemp869 + fTemp668 * (fTemp870 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp867, 4)) as usize] } - fTemp868))))} else {1.0 - (fTemp863 + fTemp668 * fTemp864 + (fTemp859 - (iTemp860) as F64) * (fTemp862 - (fTemp863 + fTemp668 * (fTemp864 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp861, 4)) as usize] } - fTemp862)))))};
			...
			let mut fTemp876: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp875, 3)) as usize] };
			let mut fTemp877: F64 = unsafe { ftbl0mydspSIG0[iTemp875 as usize] };
			let mut fTemp878: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp875, 1)) as usize] } - fTemp877;
			...
			let mut fTemp882: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp881, 3), 196607))) as usize] };
			let mut fTemp883: F64 = unsafe { ftbl0mydspSIG0[iTemp881 as usize] };
			let mut fTemp884: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp881, 1), 196607))) as usize] } - fTemp883;
			let mut iTemp885: i32 = (fTemp729 > ((if iTemp659 != 0 {fTemp883 + fTemp668 * fTemp884 + (fTemp879 - (iTemp880) as F64) * (fTemp882 - (fTemp883 + fTemp668 * (fTemp884 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp881, 4), 196607))) as usize] } - fTemp882))))} else {1.0 - (fTemp877 + fTemp668 * fTemp878 + (fTemp873 - (iTemp874) as F64) * (fTemp876 - (fTemp877 + fTemp668 * (fTemp878 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp875, 4)) as usize] } - fTemp876)))))} - fTemp871) / (1.0 - fTemp871))) as i32;
			...
			let mut fTemp893: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp892, 3)) as usize] };
			let mut fTemp894: F64 = unsafe { ftbl0mydspSIG0[iTemp892 as usize] };
			let mut fTemp895: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp892, 1)) as usize] } - fTemp894;
			...
			let mut fTemp907: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp906, 3)) as usize] };
			let mut fTemp908: F64 = unsafe { ftbl0mydspSIG0[iTemp906 as usize] };
			let mut fTemp909: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp906, 1)) as usize] } - fTemp908;
			...
			let mut fTemp913: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp912, 3), 196607))) as usize] };
			let mut fTemp914: F64 = unsafe { ftbl0mydspSIG0[iTemp912 as usize] };
			let mut fTemp915: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp912, 1), 196607))) as usize] } - fTemp914;
			let mut iTemp916: i32 = (fTemp729 > ((if iTemp659 != 0 {fTemp914 + fTemp668 * fTemp915 + (fTemp910 - (iTemp911) as F64) * (fTemp913 - (fTemp914 + fTemp668 * (fTemp915 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp912, 4), 196607))) as usize] } - fTemp913))))} else {1.0 - (fTemp908 + fTemp668 * fTemp909 + (fTemp904 - (iTemp905) as F64) * (fTemp907 - (fTemp908 + fTemp668 * (fTemp909 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp906, 4)) as usize] } - fTemp907)))))} - fTemp902) / (1.0 - fTemp902))) as i32;
			...
			let mut fTemp924: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp923, 3)) as usize] };
			let mut fTemp925: F64 = unsafe { ftbl0mydspSIG0[iTemp923 as usize] };
			let mut fTemp926: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp923, 1)) as usize] } - fTemp925;
			...
			let mut fTemp933: F64 = if iTemp659 != 0 {fTemp931 + fTemp668 * fTemp932 + (fTemp927 - (iTemp928) as F64) * (fTemp930 - (fTemp931 + fTemp668 * (fTemp932 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp929, 4)) as usize] } - fTemp930))))} else {1.0 - (fTemp925 + fTemp668 * fTemp926 + (fTemp921 - (iTemp922) as F64) * (fTemp924 - (fTemp925 + fTemp668 * (fTemp926 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp923, 4)) as usize] } - fTemp924)))))};
			...
			let mut fTemp938: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp937, 3)) as usize] };
			let mut fTemp939: F64 = unsafe { ftbl0mydspSIG0[iTemp937 as usize] };
			let mut fTemp940: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp937, 1)) as usize] } - fTemp939;
			...
			let mut fTemp944: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp943, 3), 196607))) as usize] };
			let mut fTemp945: F64 = unsafe { ftbl0mydspSIG0[iTemp943 as usize] };
			let mut fTemp946: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp943, 1), 196607))) as usize] } - fTemp945;
			let mut iTemp947: i32 = (fTemp729 > ((if iTemp659 != 0 {fTemp945 + fTemp668 * fTemp946 + (fTemp941 - (iTemp942) as F64) * (fTemp944 - (fTemp945 + fTemp668 * (fTemp946 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp943, 4), 196607))) as usize] } - fTemp944))))} else {1.0 - (fTemp939 + fTemp668 * fTemp940 + (fTemp935 - (iTemp936) as F64) * (fTemp938 - (fTemp939 + fTemp668 * (fTemp940 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp937, 4)) as usize] } - fTemp938)))))} - fTemp933) / (1.0 - fTemp933))) as i32;
			...
			let mut fTemp955: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp954, 3)) as usize] };
			let mut fTemp956: F64 = unsafe { ftbl0mydspSIG0[iTemp954 as usize] };
			let mut fTemp957: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp954, 1)) as usize] } - fTemp956;
			...
			let mut fTemp964: F64 = if iTemp659 != 0 {fTemp962 + fTemp668 * fTemp963 + (fTemp958 - (iTemp959) as F64) * (fTemp961 - (fTemp962 + fTemp668 * (fTemp963 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp960, 4)) as usize] } - fTemp961))))} else {1.0 - (fTemp956 + fTemp668 * fTemp957 + (fTemp952 - (iTemp953) as F64) * (fTemp955 - (fTemp956 + fTemp668 * (fTemp957 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp954, 4)) as usize] } - fTemp955)))))};
			...
			let mut fTemp969: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp968, 3)) as usize] };
			let mut fTemp970: F64 = unsafe { ftbl0mydspSIG0[iTemp968 as usize] };
			let mut fTemp971: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp968, 1)) as usize] } - fTemp970;
			...
			let mut fTemp975: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp974, 3), 196607))) as usize] };
			let mut fTemp976: F64 = unsafe { ftbl0mydspSIG0[iTemp974 as usize] };
			let mut fTemp977: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp974, 1), 196607))) as usize] } - fTemp976;
			let mut iTemp978: i32 = (fTemp729 > ((if iTemp659 != 0 {fTemp976 + fTemp668 * fTemp977 + (fTemp972 - (iTemp973) as F64) * (fTemp975 - (fTemp976 + fTemp668 * (fTemp977 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp974, 4), 196607))) as usize] } - fTemp975))))} else {1.0 - (fTemp970 + fTemp668 * fTemp971 + (fTemp966 - (iTemp967) as F64) * (fTemp969 - (fTemp970 + fTemp668 * (fTemp971 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp968, 4)) as usize] } - fTemp969)))))} - fTemp964) / (1.0 - fTemp964))) as i32;
			...
			let mut fTemp986: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp985, 3)) as usize] };
			let mut fTemp987: F64 = unsafe { ftbl0mydspSIG0[iTemp985 as usize] };
			let mut fTemp988: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp985, 1)) as usize] } - fTemp987;
			...
			let mut fTemp995: F64 = if iTemp659 != 0 {fTemp993 + fTemp668 * fTemp994 + (fTemp989 - (iTemp990) as F64) * (fTemp992 - (fTemp993 + fTemp668 * (fTemp994 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp991, 4)) as usize] } - fTemp992))))} else {1.0 - (fTemp987 + fTemp668 * fTemp988 + (fTemp983 - (iTemp984) as F64) * (fTemp986 - (fTemp987 + fTemp668 * (fTemp988 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp985, 4)) as usize] } - fTemp986)))))};
			...
			let mut fTemp1000: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp999, 3)) as usize] };
			let mut fTemp1001: F64 = unsafe { ftbl0mydspSIG0[iTemp999 as usize] };
			let mut fTemp1002: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp999, 1)) as usize] } - fTemp1001;
			...
			let mut fTemp1006: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1005, 3), 196607))) as usize] };
			let mut fTemp1007: F64 = unsafe { ftbl0mydspSIG0[iTemp1005 as usize] };
			let mut fTemp1008: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1005, 1), 196607))) as usize] } - fTemp1007;
			let mut iTemp1009: i32 = (fTemp729 > ((if iTemp659 != 0 {fTemp1007 + fTemp668 * fTemp1008 + (fTemp1003 - (iTemp1004) as F64) * (fTemp1006 - (fTemp1007 + fTemp668 * (fTemp1008 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1005, 4), 196607))) as usize] } - fTemp1006))))} else {1.0 - (fTemp1001 + fTemp668 * fTemp1002 + (fTemp997 - (iTemp998) as F64) * (fTemp1000 - (fTemp1001 + fTemp668 * (fTemp1002 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp999, 4)) as usize] } - fTemp1000)))))} - fTemp995) / (1.0 - fTemp995))) as i32;
			...
			let mut fTemp1017: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1016, 3)) as usize] };
			let mut fTemp1018: F64 = unsafe { ftbl0mydspSIG0[iTemp1016 as usize] };
			let mut fTemp1019: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1016, 1)) as usize] } - fTemp1018;
			...
			let mut fTemp1026: F64 = if iTemp659 != 0 {fTemp1024 + fTemp668 * fTemp1025 + (fTemp1020 - (iTemp1021) as F64) * (fTemp1023 - (fTemp1024 + fTemp668 * (fTemp1025 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1022, 4)) as usize] } - fTemp1023))))} else {1.0 - (fTemp1018 + fTemp668 * fTemp1019 + (fTemp1014 - (iTemp1015) as F64) * (fTemp1017 - (fTemp1018 + fTemp668 * (fTemp1019 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1016, 4)) as usize] } - fTemp1017)))))};
			...
			let mut fTemp1031: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1030, 3)) as usize] };
			let mut fTemp1032: F64 = unsafe { ftbl0mydspSIG0[iTemp1030 as usize] };
			let mut fTemp1033: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1030, 1)) as usize] } - fTemp1032;
			...
			let mut fTemp1037: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1036, 3), 196607))) as usize] };
			let mut fTemp1038: F64 = unsafe { ftbl0mydspSIG0[iTemp1036 as usize] };
			let mut fTemp1039: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1036, 1), 196607))) as usize] } - fTemp1038;
			let mut iTemp1040: i32 = (fTemp729 > ((if iTemp659 != 0 {fTemp1038 + fTemp668 * fTemp1039 + (fTemp1034 - (iTemp1035) as F64) * (fTemp1037 - (fTemp1038 + fTemp668 * (fTemp1039 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1036, 4), 196607))) as usize] } - fTemp1037))))} else {1.0 - (fTemp1032 + fTemp668 * fTemp1033 + (fTemp1028 - (iTemp1029) as F64) * (fTemp1031 - (fTemp1032 + fTemp668 * (fTemp1033 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1030, 4)) as usize] } - fTemp1031)))))} - fTemp1026) / (1.0 - fTemp1026))) as i32;
			...
			let mut fTemp1048: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1047, 3)) as usize] };
			let mut fTemp1049: F64 = unsafe { ftbl0mydspSIG0[iTemp1047 as usize] };
			let mut fTemp1050: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1047, 1)) as usize] } - fTemp1049;
			...
			let mut fTemp1062: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1061, 3)) as usize] };
			let mut fTemp1063: F64 = unsafe { ftbl0mydspSIG0[iTemp1061 as usize] };
			let mut fTemp1064: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1061, 1)) as usize] } - fTemp1063;
			...
			let mut fTemp1068: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1067, 3), 196607))) as usize] };
			let mut fTemp1069: F64 = unsafe { ftbl0mydspSIG0[iTemp1067 as usize] };
			let mut fTemp1070: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1067, 1), 196607))) as usize] } - fTemp1069;
			let mut iTemp1071: i32 = (fTemp729 > ((if iTemp659 != 0 {fTemp1069 + fTemp668 * fTemp1070 + (fTemp1065 - (iTemp1066) as F64) * (fTemp1068 - (fTemp1069 + fTemp668 * (fTemp1070 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1067, 4), 196607))) as usize] } - fTemp1068))))} else {1.0 - (fTemp1063 + fTemp668 * fTemp1064 + (fTemp1059 - (iTemp1060) as F64) * (fTemp1062 - (fTemp1063 + fTemp668 * (fTemp1064 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1061, 4)) as usize] } - fTemp1062)))))} - fTemp1057) / (1.0 - fTemp1057))) as i32;
			...
			let mut fTemp1079: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1078, 3)) as usize] };
			let mut fTemp1080: F64 = unsafe { ftbl0mydspSIG0[iTemp1078 as usize] };
			let mut fTemp1081: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1078, 1)) as usize] } - fTemp1080;
			...
			let mut fTemp1088: F64 = if iTemp659 != 0 {fTemp1086 + fTemp668 * fTemp1087 + (fTemp1082 - (iTemp1083) as F64) * (fTemp1085 - (fTemp1086 + fTemp668 * (fTemp1087 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1084, 4)) as usize] } - fTemp1085))))} else {1.0 - (fTemp1080 + fTemp668 * fTemp1081 + (fTemp1076 - (iTemp1077) as F64) * (fTemp1079 - (fTemp1080 + fTemp668 * (fTemp1081 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1078, 4)) as usize] } - fTemp1079)))))};
			...
			let mut fTemp1093: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1092, 3)) as usize] };
			let mut fTemp1094: F64 = unsafe { ftbl0mydspSIG0[iTemp1092 as usize] };
			let mut fTemp1095: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1092, 1)) as usize] } - fTemp1094;
			...
			let mut fTemp1099: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1098, 3), 196607))) as usize] };
			let mut fTemp1100: F64 = unsafe { ftbl0mydspSIG0[iTemp1098 as usize] };
			let mut fTemp1101: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1098, 1), 196607))) as usize] } - fTemp1100;
			...
			let mut fTemp1110: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1109, 3)) as usize] };
			let mut fTemp1111: F64 = unsafe { ftbl0mydspSIG0[iTemp1109 as usize] };
			let mut fTemp1112: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1109, 1)) as usize] } - fTemp1111;
			...
			let mut fTemp1119: F64 = if iTemp659 != 0 {fTemp1117 + fTemp668 * fTemp1118 + (fTemp1113 - (iTemp1114) as F64) * (fTemp1116 - (fTemp1117 + fTemp668 * (fTemp1118 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1115, 4)) as usize] } - fTemp1116))))} else {1.0 - (fTemp1111 + fTemp668 * fTemp1112 + (fTemp1107 - (iTemp1108) as F64) * (fTemp1110 - (fTemp1111 + fTemp668 * (fTemp1112 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1109, 4)) as usize] } - fTemp1110)))))};
			...
			let mut fTemp1124: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1123, 3)) as usize] };
			let mut fTemp1125: F64 = unsafe { ftbl0mydspSIG0[iTemp1123 as usize] };
			let mut fTemp1126: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1123, 1)) as usize] } - fTemp1125;
			...
			let mut fTemp1130: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1129, 3), 196607))) as usize] };
			let mut fTemp1131: F64 = unsafe { ftbl0mydspSIG0[iTemp1129 as usize] };
			let mut fTemp1132: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1129, 1), 196607))) as usize] } - fTemp1131;
			...
			let mut fTemp1141: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1140, 3)) as usize] };
			let mut fTemp1142: F64 = unsafe { ftbl0mydspSIG0[iTemp1140 as usize] };
			let mut fTemp1143: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1140, 1)) as usize] } - fTemp1142;
			...
			let mut fTemp1150: F64 = if iTemp659 != 0 {fTemp1148 + fTemp668 * fTemp1149 + (fTemp1144 - (iTemp1145) as F64) * (fTemp1147 - (fTemp1148 + fTemp668 * (fTemp1149 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1146, 4)) as usize] } - fTemp1147))))} else {1.0 - (fTemp1142 + fTemp668 * fTemp1143 + (fTemp1138 - (iTemp1139) as F64) * (fTemp1141 - (fTemp1142 + fTemp668 * (fTemp1143 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1140, 4)) as usize] } - fTemp1141)))))};
			...
			let mut fTemp1155: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1154, 3)) as usize] };
			let mut fTemp1156: F64 = unsafe { ftbl0mydspSIG0[iTemp1154 as usize] };
			let mut fTemp1157: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1154, 1)) as usize] } - fTemp1156;
			...
			let mut fTemp1161: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1160, 3), 196607))) as usize] };
			let mut fTemp1162: F64 = unsafe { ftbl0mydspSIG0[iTemp1160 as usize] };
			let mut fTemp1163: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1160, 1), 196607))) as usize] } - fTemp1162;
			let mut iTemp1164: i32 = (fTemp729 > ((if iTemp659 != 0 {fTemp1162 + fTemp668 * fTemp1163 + (fTemp1158 - (iTemp1159) as F64) * (fTemp1161 - (fTemp1162 + fTemp668 * (fTemp1163 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1160, 4), 196607))) as usize] } - fTemp1161))))} else {1.0 - (fTemp1156 + fTemp668 * fTemp1157 + (fTemp1152 - (iTemp1153) as F64) * (fTemp1155 - (fTemp1156 + fTemp668 * (fTemp1157 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1154, 4)) as usize] } - fTemp1155)))))} - fTemp1150) / (1.0 - fTemp1150))) as i32;
			...
			let mut fTemp1172: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1171, 3)) as usize] };
			let mut fTemp1173: F64 = unsafe { ftbl0mydspSIG0[iTemp1171 as usize] };
			let mut fTemp1174: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1171, 1)) as usize] } - fTemp1173;
			...
			let mut fTemp1181: F64 = if iTemp659 != 0 {fTemp1179 + fTemp668 * fTemp1180 + (fTemp1175 - (iTemp1176) as F64) * (fTemp1178 - (fTemp1179 + fTemp668 * (fTemp1180 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1177, 4), 196607))) as usize] } - fTemp1178))))} else {1.0 - (fTemp1173 + fTemp668 * fTemp1174 + (fTemp1169 - (iTemp1170) as F64) * (fTemp1172 - (fTemp1173 + fTemp668 * (fTemp1174 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1171, 4), 196607))) as usize] } - fTemp1172)))))};
			...
			let mut fTemp1186: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1185, 3)) as usize] };
			let mut fTemp1187: F64 = unsafe { ftbl0mydspSIG0[iTemp1185 as usize] };
			let mut fTemp1188: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1185, 1)) as usize] } - fTemp1187;
			...
			let mut fTemp1192: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1191, 3), 196607))) as usize] };
			let mut fTemp1193: F64 = unsafe { ftbl0mydspSIG0[iTemp1191 as usize] };
			let mut fTemp1194: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1191, 1), 196607))) as usize] } - fTemp1193;
			let mut iTemp1195: i32 = (fTemp729 > ((if iTemp659 != 0 {fTemp1193 + fTemp668 * fTemp1194 + (fTemp1189 - (iTemp1190) as F64) * (fTemp1192 - (fTemp1193 + fTemp668 * (fTemp1194 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1191, 4), 196607))) as usize] } - fTemp1192))))} else {1.0 - (fTemp1187 + fTemp668 * fTemp1188 + (fTemp1183 - (iTemp1184) as F64) * (fTemp1186 - (fTemp1187 + fTemp668 * (fTemp1188 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1185, 4)) as usize] } - fTemp1186)))))} - fTemp1181) / (1.0 - fTemp1181))) as i32;
			...
			let mut fTemp1200: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1199, 3)) as usize] };
			let mut fTemp1201: F64 = unsafe { ftbl0mydspSIG0[iTemp1199 as usize] };
			let mut fTemp1202: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1199, 1)) as usize] } - fTemp1201;
			...
			let mut fTemp1214: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1213, 3)) as usize] };
			let mut fTemp1215: F64 = unsafe { ftbl0mydspSIG0[iTemp1213 as usize] };
			let mut fTemp1216: F64 = unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1213, 1)) as usize] } - fTemp1215;
			...
			let mut fTemp1220: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1219, 3), 196607))) as usize] };
			let mut fTemp1221: F64 = unsafe { ftbl0mydspSIG0[iTemp1219 as usize] };
			let mut fTemp1222: F64 = unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1219, 1), 196607))) as usize] } - fTemp1221;
			let mut fTemp1223: F64 = fTemp633 + if ((0.001 * fTemp672) == 0.0) as i32 != 0 {fTemp658} else {fTemp658 * (if iTemp659 != 0 {fTemp1221 + fTemp668 * fTemp1222 + (fTemp1217 - (iTemp1218) as F64) * (fTemp1220 - (fTemp1221 + fTemp668 * (fTemp1222 - (unsafe { ftbl0mydspSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1219, 4), 196607))) as usize] } - fTemp1220))))} else {1.0 - (fTemp1215 + fTemp668 * fTemp1216 + (fTemp1211 - (iTemp1212) as F64) * (fTemp1214 - (fTemp1215 + fTemp668 * (fTemp1216 - (unsafe { ftbl0mydspSIG0[(i32::wrapping_add(iTemp1213, 4)) as usize] } - fTemp1214)))))} - fTemp1209) / (1.0 - fTemp1209)};
