...
impl LambRs96kSIG0 {
	...
	fn fillLambRs96kSIG0(&mut self, count: i32, table: &mut[F64]) {
		for i1 in 0..count {
			...
			let mut fTemp70: F64 = (0.14285714285714285 * (iTemp66 % 1835008) as F64) as i32 as F64;
			table[i1 as usize] = F64::min(1.0, F64::max(0.0, if (fTemp68 == 0.0) as i32 != 0 {0.5 * (F64::sin(1.1984270621720943e-05 * fTemp70 + 4.71238898038469) + 1.0)} else {0.5 * (F64::sin(3.141592653589793 * ((1.0 - F64::exp(-(9.231602598581689e-06 * fTemp69 * fTemp70))) / (1.0 - F64::exp(-(2.42 * fTemp69)))) + 4.71238898038469) + 1.0)}));
...
impl FaustDsp for LambRs96k {
	...
	fn compute(&mut self, count: i32, inputs: &[&[Self::T]], outputs: &mut[&mut[Self::T]]) {
		...
		for (((((input0, input1), output0), output1), output2), output3) in zipped_iterators {
			...
			let mut fTemp26: F64 = (self.iVec0[((i32::wrapping_sub(self.IOTA0, 9600)) & 16383) as usize]) as F64;
			...
			let mut fTemp41: F64 = F64::min(fTemp34, -(fSlow18 * F64::max(0.0, if (iTemp39 == 0) as i32 != 0 {0.0} else {if (iTemp39 == 1) as i32 != 0 {fSlow12 * LambRs96k_faustpower2_f(fSlow7 + fTemp40)} else {fTemp40}})));
			...
			let mut iTemp75: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, 917497), 1835007));
			let mut fTemp76: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp75, 7)) as usize] };
			...
			let mut fTemp80: F64 = fTemp77 + fTemp79 * fTemp78 + 0.5 * (fTemp76 - (fTemp77 + fTemp79 * (fTemp78 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp75, 8)) as usize] } - fTemp76))));
			...
			let mut iTemp88: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp87, 262143)))), 1835007));
			let mut fTemp89: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp88, 7)) as usize] };
			...
			let mut iTemp94: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp93, 262143)))), 1835007));
			let mut fTemp95: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp94, 7), 1835007))) as usize] };
			let mut fTemp96: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp94 as usize] };
			let mut fTemp97: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp94, 1), 1835007))) as usize] } - fTemp96;
			...
			let mut iTemp103: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp102, 262143))), iTemp100), 1835007));
			let mut fTemp104: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp103, 7), 1835007))) as usize] };
			let mut fTemp105: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp103 as usize] };
			let mut fTemp106: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp103, 1), 1835007))) as usize] } - fTemp105;
			...
			let mut iTemp110: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp100, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp109, 262143)))), 1835007));
			let mut fTemp111: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp110, 7), 1835007))) as usize] };
			let mut fTemp112: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp110 as usize] };
			let mut fTemp113: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp110, 1), 1835007))) as usize] } - fTemp112;
			let mut fTemp114: F64 = self.fRec1[1] + fTemp84;
			let mut fTemp115: F64 = 262143.0 * (1.0 - fTemp114);
			let mut iTemp116: i32 = (fTemp115) as i32;
			let mut iTemp117: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp116, 262143)))), 1835007));
			let mut fTemp118: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp117, 7)) as usize] };
			...
			let mut iTemp123: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp122, 262143)))), 1835007));
			let mut fTemp124: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp123, 7), 1835007))) as usize] };
			let mut fTemp125: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp123 as usize] };
			let mut fTemp126: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp123, 1), 1835007))) as usize] } - fTemp125;
			...
			let mut iTemp130: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp129, 262143))), iTemp74), 1835007));
			let mut fTemp131: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp130, 7)) as usize] };
			...
			let mut iTemp148: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp147, 262143)))), 1835007));
			let mut fTemp149: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp148, 7)) as usize] };
			...
			let mut iTemp154: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp153, 262143)))), 1835007));
			let mut fTemp155: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp154, 7)) as usize] };
			...
			let mut fTemp158: F64 = if iTemp65 != 0 {fTemp156 + fTemp79 * fTemp157 + (fTemp152 - (iTemp153) as F64) * (fTemp155 - (fTemp156 + fTemp79 * (fTemp157 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp154, 8)) as usize] } - fTemp155))))} else {1.0 - (fTemp150 + fTemp79 * fTemp151 + (fTemp146 - (iTemp147) as F64) * (fTemp149 - (fTemp150 + fTemp79 * (fTemp151 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp148, 8)) as usize] } - fTemp149)))))};
			let mut fTemp159: F64 = fTemp84 + fTemp145;
			let mut fTemp160: F64 = 262143.0 * (1.0 - fTemp159);
			let mut iTemp161: i32 = (fTemp160) as i32;
			let mut iTemp162: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp161, 262143)))), 1835007));
			let mut fTemp163: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp162, 7)) as usize] };
			...
			let mut iTemp168: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp167, 262143)))), 1835007));
			let mut fTemp169: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp168, 7), 1835007))) as usize] };
			let mut fTemp170: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp168 as usize] };
			let mut fTemp171: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp168, 1), 1835007))) as usize] } - fTemp170;
			let mut iTemp172: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp170 + fTemp79 * fTemp171 + (fTemp166 - (iTemp167) as F64) * (fTemp169 - (fTemp170 + fTemp79 * (fTemp171 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp168, 8), 1835007))) as usize] } - fTemp169))))} else {1.0 - (fTemp164 + fTemp79 * fTemp165 + (fTemp160 - (iTemp161) as F64) * (fTemp163 - (fTemp164 + fTemp79 * (fTemp165 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp162, 8)) as usize] } - fTemp163)))))} - fTemp158) / (1.0 - fTemp158))) as i32;
			...
			let mut iTemp179: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp178, 262143)))), 1835007));
			let mut fTemp180: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp179, 7)) as usize] };
			...
			let mut iTemp185: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp184, 262143)))), 1835007));
			let mut fTemp186: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp185, 7)) as usize] };
			...
			let mut fTemp189: F64 = if iTemp65 != 0 {fTemp187 + fTemp79 * fTemp188 + (fTemp183 - (iTemp184) as F64) * (fTemp186 - (fTemp187 + fTemp79 * (fTemp188 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp185, 8)) as usize] } - fTemp186))))} else {1.0 - (fTemp181 + fTemp79 * fTemp182 + (fTemp177 - (iTemp178) as F64) * (fTemp180 - (fTemp181 + fTemp79 * (fTemp182 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp179, 8)) as usize] } - fTemp180)))))};
			let mut fTemp190: F64 = fTemp84 + fTemp176;
			let mut fTemp191: F64 = 262143.0 * (1.0 - fTemp190);
			let mut iTemp192: i32 = (fTemp191) as i32;
			let mut iTemp193: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp192, 262143)))), 1835007));
			let mut fTemp194: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp193, 7)) as usize] };
			...
			let mut iTemp199: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp198, 262143)))), 1835007));
			let mut fTemp200: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp199, 7), 1835007))) as usize] };
			let mut fTemp201: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp199 as usize] };
			let mut fTemp202: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp199, 1), 1835007))) as usize] } - fTemp201;
			let mut iTemp203: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp201 + fTemp79 * fTemp202 + (fTemp197 - (iTemp198) as F64) * (fTemp200 - (fTemp201 + fTemp79 * (fTemp202 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp199, 8), 1835007))) as usize] } - fTemp200))))} else {1.0 - (fTemp195 + fTemp79 * fTemp196 + (fTemp191 - (iTemp192) as F64) * (fTemp194 - (fTemp195 + fTemp79 * (fTemp196 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp193, 8)) as usize] } - fTemp194)))))} - fTemp189) / (1.0 - fTemp189))) as i32;
			...
			let mut iTemp210: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp209, 262143)))), 1835007));
			let mut fTemp211: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp210, 7)) as usize] };
			...
			let mut iTemp216: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp215, 262143)))), 1835007));
			let mut fTemp217: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp216, 7)) as usize] };
			...
			let mut fTemp220: F64 = if iTemp65 != 0 {fTemp218 + fTemp79 * fTemp219 + (fTemp214 - (iTemp215) as F64) * (fTemp217 - (fTemp218 + fTemp79 * (fTemp219 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp216, 8)) as usize] } - fTemp217))))} else {1.0 - (fTemp212 + fTemp79 * fTemp213 + (fTemp208 - (iTemp209) as F64) * (fTemp211 - (fTemp212 + fTemp79 * (fTemp213 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp210, 8)) as usize] } - fTemp211)))))};
			let mut fTemp221: F64 = fTemp84 + fTemp207;
			let mut fTemp222: F64 = 262143.0 * (1.0 - fTemp221);
			let mut iTemp223: i32 = (fTemp222) as i32;
			let mut iTemp224: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp223, 262143)))), 1835007));
			let mut fTemp225: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp224, 7)) as usize] };
			...
			let mut iTemp230: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp229, 262143)))), 1835007));
			let mut fTemp231: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp230, 7), 1835007))) as usize] };
			let mut fTemp232: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp230 as usize] };
			let mut fTemp233: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp230, 1), 1835007))) as usize] } - fTemp232;
			let mut iTemp234: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp232 + fTemp79 * fTemp233 + (fTemp228 - (iTemp229) as F64) * (fTemp231 - (fTemp232 + fTemp79 * (fTemp233 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp230, 8), 1835007))) as usize] } - fTemp231))))} else {1.0 - (fTemp226 + fTemp79 * fTemp227 + (fTemp222 - (iTemp223) as F64) * (fTemp225 - (fTemp226 + fTemp79 * (fTemp227 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp224, 8)) as usize] } - fTemp225)))))} - fTemp220) / (1.0 - fTemp220))) as i32;
			...
			let mut iTemp241: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp240, 262143)))), 1835007));
			let mut fTemp242: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp241, 7)) as usize] };
			...
			let mut iTemp247: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp246, 262143)))), 1835007));
			let mut fTemp248: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp247, 7)) as usize] };
			...
			let mut fTemp251: F64 = if iTemp65 != 0 {fTemp249 + fTemp79 * fTemp250 + (fTemp245 - (iTemp246) as F64) * (fTemp248 - (fTemp249 + fTemp79 * (fTemp250 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp247, 8)) as usize] } - fTemp248))))} else {1.0 - (fTemp243 + fTemp79 * fTemp244 + (fTemp239 - (iTemp240) as F64) * (fTemp242 - (fTemp243 + fTemp79 * (fTemp244 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp241, 8)) as usize] } - fTemp242)))))};
			let mut fTemp252: F64 = fTemp84 + fTemp238;
			let mut fTemp253: F64 = 262143.0 * (1.0 - fTemp252);
			let mut iTemp254: i32 = (fTemp253) as i32;
			let mut iTemp255: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp254, 262143)))), 1835007));
			let mut fTemp256: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp255, 7)) as usize] };
			...
			let mut iTemp261: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp260, 262143)))), 1835007));
			let mut fTemp262: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp261, 7), 1835007))) as usize] };
			let mut fTemp263: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp261 as usize] };
			let mut fTemp264: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp261, 1), 1835007))) as usize] } - fTemp263;
			let mut iTemp265: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp263 + fTemp79 * fTemp264 + (fTemp259 - (iTemp260) as F64) * (fTemp262 - (fTemp263 + fTemp79 * (fTemp264 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp261, 8), 1835007))) as usize] } - fTemp262))))} else {1.0 - (fTemp257 + fTemp79 * fTemp258 + (fTemp253 - (iTemp254) as F64) * (fTemp256 - (fTemp257 + fTemp79 * (fTemp258 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp255, 8)) as usize] } - fTemp256)))))} - fTemp251) / (1.0 - fTemp251))) as i32;
			...
			let mut iTemp272: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp271, 262143)))), 1835007));
			let mut fTemp273: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp272, 7)) as usize] };
			...
			let mut iTemp278: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp277, 262143)))), 1835007));
			let mut fTemp279: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp278, 7)) as usize] };
			...
			let mut fTemp282: F64 = if iTemp65 != 0 {fTemp280 + fTemp79 * fTemp281 + (fTemp276 - (iTemp277) as F64) * (fTemp279 - (fTemp280 + fTemp79 * (fTemp281 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp278, 8)) as usize] } - fTemp279))))} else {1.0 - (fTemp274 + fTemp79 * fTemp275 + (fTemp270 - (iTemp271) as F64) * (fTemp273 - (fTemp274 + fTemp79 * (fTemp275 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp272, 8)) as usize] } - fTemp273)))))};
			let mut fTemp283: F64 = fTemp84 + fTemp269;
			let mut fTemp284: F64 = 262143.0 * (1.0 - fTemp283);
			let mut iTemp285: i32 = (fTemp284) as i32;
			let mut iTemp286: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp285, 262143)))), 1835007));
			let mut fTemp287: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp286, 7)) as usize] };
			...
			let mut iTemp292: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp291, 262143)))), 1835007));
			let mut fTemp293: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp292, 7), 1835007))) as usize] };
			let mut fTemp294: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp292 as usize] };
			let mut fTemp295: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp292, 1), 1835007))) as usize] } - fTemp294;
			let mut iTemp296: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp294 + fTemp79 * fTemp295 + (fTemp290 - (iTemp291) as F64) * (fTemp293 - (fTemp294 + fTemp79 * (fTemp295 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp292, 8), 1835007))) as usize] } - fTemp293))))} else {1.0 - (fTemp288 + fTemp79 * fTemp289 + (fTemp284 - (iTemp285) as F64) * (fTemp287 - (fTemp288 + fTemp79 * (fTemp289 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp286, 8)) as usize] } - fTemp287)))))} - fTemp282) / (1.0 - fTemp282))) as i32;
			...
			let mut iTemp303: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp302, 262143)))), 1835007));
			let mut fTemp304: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp303, 7)) as usize] };
			...
			let mut iTemp309: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp308, 262143)))), 1835007));
			let mut fTemp310: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp309, 7)) as usize] };
			...
			let mut fTemp313: F64 = if iTemp65 != 0 {fTemp311 + fTemp79 * fTemp312 + (fTemp307 - (iTemp308) as F64) * (fTemp310 - (fTemp311 + fTemp79 * (fTemp312 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp309, 8)) as usize] } - fTemp310))))} else {1.0 - (fTemp305 + fTemp79 * fTemp306 + (fTemp301 - (iTemp302) as F64) * (fTemp304 - (fTemp305 + fTemp79 * (fTemp306 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp303, 8)) as usize] } - fTemp304)))))};
			let mut fTemp314: F64 = fTemp84 + fTemp300;
			let mut fTemp315: F64 = 262143.0 * (1.0 - fTemp314);
			let mut iTemp316: i32 = (fTemp315) as i32;
			let mut iTemp317: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp316, 262143)))), 1835007));
			let mut fTemp318: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp317, 7)) as usize] };
			...
			let mut iTemp323: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp322, 262143)))), 1835007));
			let mut fTemp324: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp323, 7), 1835007))) as usize] };
			let mut fTemp325: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp323 as usize] };
			let mut fTemp326: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp323, 1), 1835007))) as usize] } - fTemp325;
			let mut iTemp327: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp325 + fTemp79 * fTemp326 + (fTemp321 - (iTemp322) as F64) * (fTemp324 - (fTemp325 + fTemp79 * (fTemp326 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp323, 8), 1835007))) as usize] } - fTemp324))))} else {1.0 - (fTemp319 + fTemp79 * fTemp320 + (fTemp315 - (iTemp316) as F64) * (fTemp318 - (fTemp319 + fTemp79 * (fTemp320 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp317, 8)) as usize] } - fTemp318)))))} - fTemp313) / (1.0 - fTemp313))) as i32;
			...
			let mut iTemp334: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp333, 262143)))), 1835007));
			let mut fTemp335: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp334, 7)) as usize] };
			...
			let mut iTemp340: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp339, 262143)))), 1835007));
			let mut fTemp341: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp340, 7)) as usize] };
			...
			let mut fTemp344: F64 = if iTemp65 != 0 {fTemp342 + fTemp79 * fTemp343 + (fTemp338 - (iTemp339) as F64) * (fTemp341 - (fTemp342 + fTemp79 * (fTemp343 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp340, 8)) as usize] } - fTemp341))))} else {1.0 - (fTemp336 + fTemp79 * fTemp337 + (fTemp332 - (iTemp333) as F64) * (fTemp335 - (fTemp336 + fTemp79 * (fTemp337 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp334, 8)) as usize] } - fTemp335)))))};
			let mut fTemp345: F64 = fTemp84 + fTemp331;
			let mut fTemp346: F64 = 262143.0 * (1.0 - fTemp345);
			let mut iTemp347: i32 = (fTemp346) as i32;
			let mut iTemp348: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp347, 262143)))), 1835007));
			let mut fTemp349: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp348, 7)) as usize] };
			...
			let mut iTemp354: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp353, 262143)))), 1835007));
			let mut fTemp355: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp354, 7), 1835007))) as usize] };
			let mut fTemp356: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp354 as usize] };
			let mut fTemp357: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp354, 1), 1835007))) as usize] } - fTemp356;
			let mut iTemp358: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp356 + fTemp79 * fTemp357 + (fTemp352 - (iTemp353) as F64) * (fTemp355 - (fTemp356 + fTemp79 * (fTemp357 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp354, 8), 1835007))) as usize] } - fTemp355))))} else {1.0 - (fTemp350 + fTemp79 * fTemp351 + (fTemp346 - (iTemp347) as F64) * (fTemp349 - (fTemp350 + fTemp79 * (fTemp351 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp348, 8)) as usize] } - fTemp349)))))} - fTemp344) / (1.0 - fTemp344))) as i32;
			...
			let mut iTemp365: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp364, 262143)))), 1835007));
			let mut fTemp366: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp365, 7)) as usize] };
			...
			let mut iTemp371: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp370, 262143)))), 1835007));
			let mut fTemp372: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp371, 7)) as usize] };
			...
			let mut fTemp375: F64 = if iTemp65 != 0 {fTemp373 + fTemp79 * fTemp374 + (fTemp369 - (iTemp370) as F64) * (fTemp372 - (fTemp373 + fTemp79 * (fTemp374 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp371, 8)) as usize] } - fTemp372))))} else {1.0 - (fTemp367 + fTemp79 * fTemp368 + (fTemp363 - (iTemp364) as F64) * (fTemp366 - (fTemp367 + fTemp79 * (fTemp368 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp365, 8)) as usize] } - fTemp366)))))};
			let mut fTemp376: F64 = fTemp84 + fTemp362;
			let mut fTemp377: F64 = 262143.0 * (1.0 - fTemp376);
			let mut iTemp378: i32 = (fTemp377) as i32;
			let mut iTemp379: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp378, 262143)))), 1835007));
			let mut fTemp380: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp379, 7)) as usize] };
			...
			let mut iTemp385: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp384, 262143)))), 1835007));
			let mut fTemp386: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp385, 7), 1835007))) as usize] };
			let mut fTemp387: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp385 as usize] };
			let mut fTemp388: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp385, 1), 1835007))) as usize] } - fTemp387;
			let mut iTemp389: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp387 + fTemp79 * fTemp388 + (fTemp383 - (iTemp384) as F64) * (fTemp386 - (fTemp387 + fTemp79 * (fTemp388 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp385, 8), 1835007))) as usize] } - fTemp386))))} else {1.0 - (fTemp381 + fTemp79 * fTemp382 + (fTemp377 - (iTemp378) as F64) * (fTemp380 - (fTemp381 + fTemp79 * (fTemp382 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp379, 8)) as usize] } - fTemp380)))))} - fTemp375) / (1.0 - fTemp375))) as i32;
			...
			let mut iTemp396: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp395, 262143)))), 1835007));
			let mut fTemp397: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp396, 7)) as usize] };
			...
			let mut iTemp402: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp401, 262143)))), 1835007));
			let mut fTemp403: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp402, 7)) as usize] };
			...
			let mut fTemp406: F64 = if iTemp65 != 0 {fTemp404 + fTemp79 * fTemp405 + (fTemp400 - (iTemp401) as F64) * (fTemp403 - (fTemp404 + fTemp79 * (fTemp405 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp402, 8)) as usize] } - fTemp403))))} else {1.0 - (fTemp398 + fTemp79 * fTemp399 + (fTemp394 - (iTemp395) as F64) * (fTemp397 - (fTemp398 + fTemp79 * (fTemp399 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp396, 8)) as usize] } - fTemp397)))))};
			let mut fTemp407: F64 = fTemp84 + fTemp393;
			let mut fTemp408: F64 = 262143.0 * (1.0 - fTemp407);
			let mut iTemp409: i32 = (fTemp408) as i32;
			let mut iTemp410: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp409, 262143)))), 1835007));
			let mut fTemp411: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp410, 7)) as usize] };
			...
			let mut iTemp416: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp415, 262143)))), 1835007));
			let mut fTemp417: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp416, 7), 1835007))) as usize] };
			let mut fTemp418: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp416 as usize] };
			let mut fTemp419: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp416, 1), 1835007))) as usize] } - fTemp418;
			let mut iTemp420: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp418 + fTemp79 * fTemp419 + (fTemp414 - (iTemp415) as F64) * (fTemp417 - (fTemp418 + fTemp79 * (fTemp419 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp416, 8), 1835007))) as usize] } - fTemp417))))} else {1.0 - (fTemp412 + fTemp79 * fTemp413 + (fTemp408 - (iTemp409) as F64) * (fTemp411 - (fTemp412 + fTemp79 * (fTemp413 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp410, 8)) as usize] } - fTemp411)))))} - fTemp406) / (1.0 - fTemp406))) as i32;
			...
			let mut iTemp427: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp426, 262143)))), 1835007));
			let mut fTemp428: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp427, 7)) as usize] };
			...
			let mut iTemp433: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp432, 262143)))), 1835007));
			let mut fTemp434: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp433, 7)) as usize] };
			...
			let mut fTemp437: F64 = if iTemp65 != 0 {fTemp435 + fTemp79 * fTemp436 + (fTemp431 - (iTemp432) as F64) * (fTemp434 - (fTemp435 + fTemp79 * (fTemp436 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp433, 8)) as usize] } - fTemp434))))} else {1.0 - (fTemp429 + fTemp79 * fTemp430 + (fTemp425 - (iTemp426) as F64) * (fTemp428 - (fTemp429 + fTemp79 * (fTemp430 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp427, 8)) as usize] } - fTemp428)))))};
			let mut fTemp438: F64 = fTemp84 + fTemp424;
			let mut fTemp439: F64 = 262143.0 * (1.0 - fTemp438);
			let mut iTemp440: i32 = (fTemp439) as i32;
			let mut iTemp441: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp440, 262143)))), 1835007));
			let mut fTemp442: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp441, 7)) as usize] };
			...
			let mut iTemp447: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp446, 262143)))), 1835007));
			let mut fTemp448: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp447, 7), 1835007))) as usize] };
			let mut fTemp449: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp447 as usize] };
			let mut fTemp450: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp447, 1), 1835007))) as usize] } - fTemp449;
			let mut iTemp451: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp449 + fTemp79 * fTemp450 + (fTemp445 - (iTemp446) as F64) * (fTemp448 - (fTemp449 + fTemp79 * (fTemp450 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp447, 8), 1835007))) as usize] } - fTemp448))))} else {1.0 - (fTemp443 + fTemp79 * fTemp444 + (fTemp439 - (iTemp440) as F64) * (fTemp442 - (fTemp443 + fTemp79 * (fTemp444 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp441, 8)) as usize] } - fTemp442)))))} - fTemp437) / (1.0 - fTemp437))) as i32;
			...
			let mut iTemp458: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp457, 262143)))), 1835007));
			let mut fTemp459: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp458, 7)) as usize] };
			...
			let mut iTemp464: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp463, 262143)))), 1835007));
			let mut fTemp465: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp464, 7)) as usize] };
			...
			let mut fTemp468: F64 = if iTemp65 != 0 {fTemp466 + fTemp79 * fTemp467 + (fTemp462 - (iTemp463) as F64) * (fTemp465 - (fTemp466 + fTemp79 * (fTemp467 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp464, 8)) as usize] } - fTemp465))))} else {1.0 - (fTemp460 + fTemp79 * fTemp461 + (fTemp456 - (iTemp457) as F64) * (fTemp459 - (fTemp460 + fTemp79 * (fTemp461 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp458, 8)) as usize] } - fTemp459)))))};
			let mut fTemp469: F64 = fTemp84 + fTemp455;
			let mut fTemp470: F64 = 262143.0 * (1.0 - fTemp469);
			let mut iTemp471: i32 = (fTemp470) as i32;
			let mut iTemp472: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp471, 262143)))), 1835007));
			let mut fTemp473: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp472, 7)) as usize] };
			...
			let mut iTemp478: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp477, 262143)))), 1835007));
			let mut fTemp479: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp478, 7), 1835007))) as usize] };
			let mut fTemp480: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp478 as usize] };
			let mut fTemp481: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp478, 1), 1835007))) as usize] } - fTemp480;
			let mut iTemp482: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp480 + fTemp79 * fTemp481 + (fTemp476 - (iTemp477) as F64) * (fTemp479 - (fTemp480 + fTemp79 * (fTemp481 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp478, 8), 1835007))) as usize] } - fTemp479))))} else {1.0 - (fTemp474 + fTemp79 * fTemp475 + (fTemp470 - (iTemp471) as F64) * (fTemp473 - (fTemp474 + fTemp79 * (fTemp475 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp472, 8)) as usize] } - fTemp473)))))} - fTemp468) / (1.0 - fTemp468))) as i32;
			...
			let mut iTemp489: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp488, 262143)))), 1835007));
			let mut fTemp490: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp489, 7)) as usize] };
			...
			let mut iTemp495: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp494, 262143)))), 1835007));
			let mut fTemp496: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp495, 7)) as usize] };
			...
			let mut fTemp499: F64 = if iTemp65 != 0 {fTemp497 + fTemp79 * fTemp498 + (fTemp493 - (iTemp494) as F64) * (fTemp496 - (fTemp497 + fTemp79 * (fTemp498 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp495, 8)) as usize] } - fTemp496))))} else {1.0 - (fTemp491 + fTemp79 * fTemp492 + (fTemp487 - (iTemp488) as F64) * (fTemp490 - (fTemp491 + fTemp79 * (fTemp492 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp489, 8)) as usize] } - fTemp490)))))};
			let mut fTemp500: F64 = fTemp84 + fTemp486;
			let mut fTemp501: F64 = 262143.0 * (1.0 - fTemp500);
			let mut iTemp502: i32 = (fTemp501) as i32;
			let mut iTemp503: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp502, 262143)))), 1835007));
			let mut fTemp504: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp503, 7)) as usize] };
			...
			let mut iTemp509: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp508, 262143)))), 1835007));
			let mut fTemp510: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp509, 7), 1835007))) as usize] };
			let mut fTemp511: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp509 as usize] };
			let mut fTemp512: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp509, 1), 1835007))) as usize] } - fTemp511;
			let mut iTemp513: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp511 + fTemp79 * fTemp512 + (fTemp507 - (iTemp508) as F64) * (fTemp510 - (fTemp511 + fTemp79 * (fTemp512 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp509, 8), 1835007))) as usize] } - fTemp510))))} else {1.0 - (fTemp505 + fTemp79 * fTemp506 + (fTemp501 - (iTemp502) as F64) * (fTemp504 - (fTemp505 + fTemp79 * (fTemp506 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp503, 8)) as usize] } - fTemp504)))))} - fTemp499) / (1.0 - fTemp499))) as i32;
			...
			let mut iTemp520: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp519, 262143)))), 1835007));
			let mut fTemp521: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp520, 7)) as usize] };
			...
			let mut iTemp526: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp525, 262143)))), 1835007));
			let mut fTemp527: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp526, 7)) as usize] };
			...
			let mut fTemp530: F64 = if iTemp65 != 0 {fTemp528 + fTemp79 * fTemp529 + (fTemp524 - (iTemp525) as F64) * (fTemp527 - (fTemp528 + fTemp79 * (fTemp529 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp526, 8)) as usize] } - fTemp527))))} else {1.0 - (fTemp522 + fTemp79 * fTemp523 + (fTemp518 - (iTemp519) as F64) * (fTemp521 - (fTemp522 + fTemp79 * (fTemp523 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp520, 8)) as usize] } - fTemp521)))))};
			let mut fTemp531: F64 = fTemp84 + fTemp517;
			let mut fTemp532: F64 = 262143.0 * (1.0 - fTemp531);
			let mut iTemp533: i32 = (fTemp532) as i32;
			let mut iTemp534: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp533, 262143)))), 1835007));
			let mut fTemp535: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp534, 7)) as usize] };
			...
			let mut iTemp540: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp539, 262143)))), 1835007));
			let mut fTemp541: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp540, 7), 1835007))) as usize] };
			let mut fTemp542: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp540 as usize] };
			let mut fTemp543: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp540, 1), 1835007))) as usize] } - fTemp542;
			let mut iTemp544: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp542 + fTemp79 * fTemp543 + (fTemp538 - (iTemp539) as F64) * (fTemp541 - (fTemp542 + fTemp79 * (fTemp543 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp540, 8), 1835007))) as usize] } - fTemp541))))} else {1.0 - (fTemp536 + fTemp79 * fTemp537 + (fTemp532 - (iTemp533) as F64) * (fTemp535 - (fTemp536 + fTemp79 * (fTemp537 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp534, 8)) as usize] } - fTemp535)))))} - fTemp530) / (1.0 - fTemp530))) as i32;
			...
			let mut iTemp551: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp550, 262143)))), 1835007));
			let mut fTemp552: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp551, 7)) as usize] };
			...
			let mut iTemp557: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp556, 262143)))), 1835007));
			let mut fTemp558: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp557, 7)) as usize] };
			...
			let mut fTemp561: F64 = if iTemp65 != 0 {fTemp559 + fTemp79 * fTemp560 + (fTemp555 - (iTemp556) as F64) * (fTemp558 - (fTemp559 + fTemp79 * (fTemp560 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp557, 8)) as usize] } - fTemp558))))} else {1.0 - (fTemp553 + fTemp79 * fTemp554 + (fTemp549 - (iTemp550) as F64) * (fTemp552 - (fTemp553 + fTemp79 * (fTemp554 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp551, 8)) as usize] } - fTemp552)))))};
			let mut fTemp562: F64 = fTemp84 + fTemp548;
			let mut fTemp563: F64 = 262143.0 * (1.0 - fTemp562);
			let mut iTemp564: i32 = (fTemp563) as i32;
			let mut iTemp565: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp564, 262143)))), 1835007));
			let mut fTemp566: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp565, 7)) as usize] };
			...
			let mut iTemp571: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp570, 262143)))), 1835007));
			let mut fTemp572: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp571, 7), 1835007))) as usize] };
			let mut fTemp573: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp571 as usize] };
			let mut fTemp574: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp571, 1), 1835007))) as usize] } - fTemp573;
			let mut iTemp575: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp573 + fTemp79 * fTemp574 + (fTemp569 - (iTemp570) as F64) * (fTemp572 - (fTemp573 + fTemp79 * (fTemp574 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp571, 8), 1835007))) as usize] } - fTemp572))))} else {1.0 - (fTemp567 + fTemp79 * fTemp568 + (fTemp563 - (iTemp564) as F64) * (fTemp566 - (fTemp567 + fTemp79 * (fTemp568 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp565, 8)) as usize] } - fTemp566)))))} - fTemp561) / (1.0 - fTemp561))) as i32;
			...
			let mut iTemp582: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp581, 262143)))), 1835007));
			let mut fTemp583: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp582, 7)) as usize] };
			...
			let mut iTemp588: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp587, 262143)))), 1835007));
			let mut fTemp589: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp588, 7)) as usize] };
			...
			let mut iTemp602: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp601, 262143)))), 1835007));
			let mut fTemp603: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp602, 7), 1835007))) as usize] };
			let mut fTemp604: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp602 as usize] };
			let mut fTemp605: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp602, 1), 1835007))) as usize] } - fTemp604;
			let mut iTemp606: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp604 + fTemp79 * fTemp605 + (fTemp600 - (iTemp601) as F64) * (fTemp603 - (fTemp604 + fTemp79 * (fTemp605 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp602, 8), 1835007))) as usize] } - fTemp603))))} else {1.0 - (fTemp598 + fTemp79 * fTemp599 + (fTemp594 - (iTemp595) as F64) * (fTemp597 - (fTemp598 + fTemp79 * (fTemp599 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp596, 8)) as usize] } - fTemp597)))))} - fTemp592) / (1.0 - fTemp592))) as i32;
			...
			let mut iTemp613: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp612, 262143)))), 1835007));
			let mut fTemp614: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp613, 7)) as usize] };
			...
			let mut iTemp619: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp618, 262143)))), 1835007));
			let mut fTemp620: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp619, 7)) as usize] };
			...
			let mut fTemp623: F64 = if iTemp65 != 0 {fTemp621 + fTemp79 * fTemp622 + (fTemp617 - (iTemp618) as F64) * (fTemp620 - (fTemp621 + fTemp79 * (fTemp622 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp619, 8)) as usize] } - fTemp620))))} else {1.0 - (fTemp615 + fTemp79 * fTemp616 + (fTemp611 - (iTemp612) as F64) * (fTemp614 - (fTemp615 + fTemp79 * (fTemp616 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp613, 8)) as usize] } - fTemp614)))))};
			let mut fTemp624: F64 = fTemp84 + fTemp610;
			let mut fTemp625: F64 = 262143.0 * (1.0 - fTemp624);
			let mut iTemp626: i32 = (fTemp625) as i32;
			let mut iTemp627: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp626, 262143)))), 1835007));
			let mut fTemp628: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp627, 7)) as usize] };
			...
			let mut iTemp644: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp643, 262143)))), 1835007));
			let mut fTemp645: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp644, 7)) as usize] };
			...
			let mut iTemp650: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp649, 262143)))), 1835007));
			let mut fTemp651: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp650, 7)) as usize] };
			...
			let mut fTemp654: F64 = if iTemp65 != 0 {fTemp652 + fTemp79 * fTemp653 + (fTemp648 - (iTemp649) as F64) * (fTemp651 - (fTemp652 + fTemp79 * (fTemp653 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp650, 8), 1835007))) as usize] } - fTemp651))))} else {1.0 - (fTemp646 + fTemp79 * fTemp647 + (fTemp642 - (iTemp643) as F64) * (fTemp645 - (fTemp646 + fTemp79 * (fTemp647 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp644, 8), 1835007))) as usize] } - fTemp645)))))};
			let mut fTemp655: F64 = fTemp84 + fTemp641;
			let mut fTemp656: F64 = 262143.0 * (1.0 - fTemp655);
			let mut iTemp657: i32 = (fTemp656) as i32;
			let mut iTemp658: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp657, 262143)))), 1835007));
			let mut fTemp659: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp658, 7)) as usize] };
			...
			let mut iTemp664: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp663, 262143)))), 1835007));
			let mut fTemp665: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp664, 7), 1835007))) as usize] };
			let mut fTemp666: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp664 as usize] };
			let mut fTemp667: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp664, 1), 1835007))) as usize] } - fTemp666;
			let mut iTemp668: i32 = (fTemp140 > ((if iTemp65 != 0 {fTemp666 + fTemp79 * fTemp667 + (fTemp662 - (iTemp663) as F64) * (fTemp665 - (fTemp666 + fTemp79 * (fTemp667 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp664, 8), 1835007))) as usize] } - fTemp665))))} else {1.0 - (fTemp660 + fTemp79 * fTemp661 + (fTemp656 - (iTemp657) as F64) * (fTemp659 - (fTemp660 + fTemp79 * (fTemp661 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp658, 8)) as usize] } - fTemp659)))))} - fTemp654) / (1.0 - fTemp654))) as i32;
			let mut fTemp669: F64 = F64::min(1.0, F64::max(0.0, 0.5 * (if iTemp668 != 0 {fTemp641} else {fTemp639} + if iTemp668 != 0 {fTemp638} else {fTemp641})));
			self.fRec1[0] = fTemp669;
			let mut fTemp670: F64 = 262143.0 * (1.0 - fTemp669);
			let mut iTemp671: i32 = (fTemp670) as i32;
			let mut iTemp672: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp671, 262143)))), 1835007));
			let mut fTemp673: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp672, 7)) as usize] };
			let mut fTemp674: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp672 as usize] };
			let mut fTemp675: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp672, 1)) as usize] } - fTemp674;
			let mut fTemp676: F64 = 262143.0 * fTemp669;
			let mut iTemp677: i32 = (fTemp676) as i32;
			let mut iTemp678: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp677, 262143)))), 1835007));
			let mut fTemp679: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp678, 7)) as usize] };
			let mut fTemp680: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp678 as usize] };
			let mut fTemp681: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp678, 1)) as usize] } - fTemp680;
			let mut fTemp682: F64 = if iTemp65 != 0 {fTemp680 + fTemp79 * fTemp681 + (fTemp676 - (iTemp677) as F64) * (fTemp679 - (fTemp680 + fTemp79 * (fTemp681 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp678, 8), 1835007))) as usize] } - fTemp679))))} else {1.0 - (fTemp674 + fTemp79 * fTemp675 + (fTemp670 - (iTemp671) as F64) * (fTemp673 - (fTemp674 + fTemp79 * (fTemp675 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp672, 8), 1835007))) as usize] } - fTemp673)))))};
			let mut fTemp683: F64 = fTemp84 + fTemp669;
			let mut fTemp684: F64 = 262143.0 * (1.0 - fTemp683);
			let mut iTemp685: i32 = (fTemp684) as i32;
			let mut iTemp686: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp685, 262143)))), 1835007));
			let mut fTemp687: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp686, 7)) as usize] };
			let mut fTemp688: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp686 as usize] };
			let mut fTemp689: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp686, 1)) as usize] } - fTemp688;
			let mut fTemp690: F64 = 262143.0 * fTemp683;
			let mut iTemp691: i32 = (fTemp690) as i32;
			let mut iTemp692: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp74, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp691, 262143)))), 1835007));
			let mut fTemp693: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp692, 7), 1835007))) as usize] };
			let mut fTemp694: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp692 as usize] };
			let mut fTemp695: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp692, 1), 1835007))) as usize] } - fTemp694;
			let mut fTemp696: F64 = self.fRec2[1] + if ((0.001 * fTemp83) == 0.0) as i32 != 0 {fTemp64} else {fTemp64 * (if iTemp65 != 0 {fTemp694 + fTemp79 * fTemp695 + (fTemp690 - (iTemp691) as F64) * (fTemp693 - (fTemp694 + fTemp79 * (fTemp695 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp692, 8), 1835007))) as usize] } - fTemp693))))} else {1.0 - (fTemp688 + fTemp79 * fTemp689 + (fTemp684 - (iTemp685) as F64) * (fTemp687 - (fTemp688 + fTemp79 * (fTemp689 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp686, 8)) as usize] } - fTemp687)))))} - fTemp682) / (1.0 - fTemp682)};
			self.fRec2[0] = if iTemp82 != 0 {F64::min(fTemp696, self.fRec2[1])} else {F64::max(fTemp696, self.fRec2[1])};
			self.fVec33[(self.IOTA0 & 16383) as usize] = F64::powf(1e+01, 0.05 * self.fRec2[0]);
			let mut fTemp697: F64 = self.fVec33[((i32::wrapping_sub(self.IOTA0, iSlow74)) & 16383) as usize];
			self.fRec14[0] = fSlow76 + self.fConst4 * self.fRec14[1];
			*output0 = 0.5 * self.fVec1[((i32::wrapping_sub(self.IOTA0, iSlow75)) & 32767) as usize] * fTemp2 + self.fRec14[0] * self.fVec2[((i32::wrapping_sub(self.IOTA0, iSlow75)) & 32767) as usize] * fTemp697 * fTemp4;
			let mut fTemp698: F64 = fTemp36 + fSlow17 * (fTemp37 - fTemp36);
			let mut iTemp699: i32 = ((fTemp698 > fSlow11) as i32) + ((fTemp698 > fSlow9) as i32);
			let mut fTemp700: F64 = fTemp698 - fSlow8;
			let mut fTemp701: F64 = F64::min(fTemp34, -(fSlow18 * F64::max(0.0, if (iTemp699 == 0) as i32 != 0 {0.0} else {if (iTemp699 == 1) as i32 != 0 {fSlow12 * LambRs96k_faustpower2_f(fSlow7 + fTemp700)} else {fTemp700}})));
			self.fVec34[(self.IOTA0 & 16383) as usize] = fTemp701;
			let mut fTemp702: F64 = F64::min(fTemp701, self.fVec34[((i32::wrapping_sub(self.IOTA0, 1)) & 16383) as usize]);
			...
			let mut iTemp730: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, 917497), 1835007));
			let mut fTemp731: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp730, 7)) as usize] };
			let mut fTemp732: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp730 as usize] };
			let mut fTemp733: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp730, 1)) as usize] } - fTemp732;
			let mut fTemp734: F64 = fTemp727 - (iTemp728) as F64;
			let mut fTemp735: F64 = fTemp732 + fTemp734 * fTemp733 + 0.5 * (fTemp731 - (fTemp732 + fTemp734 * (fTemp733 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp730, 8)) as usize] } - fTemp731))));
			let mut fTemp736: F64 = if iTemp725 != 0 {fTemp735} else {1.0 - fTemp735};
			...
			let mut iTemp743: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp742, 262143)))), 1835007));
			let mut fTemp744: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp743, 7)) as usize] };
			let mut fTemp745: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp743 as usize] };
			let mut fTemp746: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp743, 1)) as usize] } - fTemp745;
			let mut fTemp747: F64 = 262143.0 * fTemp740;
			let mut iTemp748: i32 = (fTemp747) as i32;
			let mut iTemp749: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp748, 262143)))), 1835007));
			let mut fTemp750: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp749, 7), 1835007))) as usize] };
			let mut fTemp751: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp749 as usize] };
			let mut fTemp752: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp749, 1), 1835007))) as usize] } - fTemp751;
			...
			let mut iTemp758: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp757, 262143))), iTemp755), 1835007));
			let mut fTemp759: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp758, 7), 1835007))) as usize] };
			let mut fTemp760: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp758 as usize] };
			let mut fTemp761: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp758, 1), 1835007))) as usize] } - fTemp760;
			...
			let mut iTemp765: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp755, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp764, 262143)))), 1835007));
			let mut fTemp766: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp765, 7), 1835007))) as usize] };
			let mut fTemp767: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp765 as usize] };
			let mut fTemp768: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp765, 1), 1835007))) as usize] } - fTemp767;
			...
			let mut iTemp772: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp771, 262143)))), 1835007));
			let mut fTemp773: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp772, 7)) as usize] };
			...
			let mut iTemp778: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp777, 262143)))), 1835007));
			let mut fTemp779: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp778, 7), 1835007))) as usize] };
			let mut fTemp780: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp778 as usize] };
			let mut fTemp781: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp778, 1), 1835007))) as usize] } - fTemp780;
			let mut fTemp782: F64 = self.fRec15[1] + self.fConst10 * (1.0 / fTemp738 + 1.0 / self.fVec61[1]);
			let mut fTemp783: F64 = 262143.0 * (1.0 - fTemp782);
			let mut iTemp784: i32 = (fTemp783) as i32;
			let mut iTemp785: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp784, 262143))), iTemp729), 1835007));
			let mut fTemp786: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp785, 7)) as usize] };
			let mut fTemp787: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp785 as usize] };
			let mut fTemp788: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp785, 1)) as usize] } - fTemp787;
			let mut fTemp789: F64 = 262143.0 * fTemp782;
			let mut iTemp790: i32 = (fTemp789) as i32;
			let mut iTemp791: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp790, 262143)))), 1835007));
			let mut fTemp792: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp791, 7), 1835007))) as usize] };
			let mut fTemp793: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp791 as usize] };
			let mut fTemp794: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp791, 1), 1835007))) as usize] } - fTemp793;
			let mut fTemp795: F64 = (if iTemp725 != 0 {fTemp793 + fTemp734 * fTemp794 + (fTemp789 - (iTemp790) as F64) * (fTemp792 - (fTemp793 + fTemp734 * (fTemp794 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp791, 8), 1835007))) as usize] } - fTemp792))))} else {1.0 - (fTemp787 + fTemp734 * fTemp788 + (fTemp783 - (iTemp784) as F64) * (fTemp786 - (fTemp787 + fTemp734 * (fTemp788 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp785, 8)) as usize] } - fTemp786)))))} - if iTemp725 != 0 {fTemp780 + fTemp734 * fTemp781 + (fTemp776 - (iTemp777) as F64) * (fTemp779 - (fTemp780 + fTemp734 * (fTemp781 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp778, 8), 1835007))) as usize] } - fTemp779))))} else {1.0 - (fTemp774 + fTemp734 * fTemp775 + (fTemp770 - (iTemp771) as F64) * (fTemp773 - (fTemp774 + fTemp734 * (fTemp775 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp772, 8)) as usize] } - fTemp773)))))}) * self.fVec59[1] / (fTemp724 * (1.0 - if iTemp725 != 0 {fTemp767 + fTemp762 * fTemp768 + (fTemp763 - (iTemp764) as F64) * (fTemp766 - (fTemp767 + fTemp762 * (fTemp768 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp765, 8), 1835007))) as usize] } - fTemp766))))} else {1.0 - (fTemp760 + fTemp762 * fTemp761 + (fTemp756 - (iTemp757) as F64) * (fTemp759 - (fTemp760 + fTemp762 * (fTemp761 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp758, 8), 1835007))) as usize] } - fTemp759)))))}));
			let mut iTemp796: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp751 + fTemp734 * fTemp752 + (fTemp747 - (iTemp748) as F64) * (fTemp750 - (fTemp751 + fTemp734 * (fTemp752 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp749, 8), 1835007))) as usize] } - fTemp750))))} else {1.0 - (fTemp745 + fTemp734 * fTemp746 + (fTemp741 - (iTemp742) as F64) * (fTemp744 - (fTemp745 + fTemp734 * (fTemp746 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp743, 8)) as usize] } - fTemp744)))))} - fTemp736) / (1.0 - fTemp736))) as i32;
			...
			let mut iTemp803: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp802, 262143)))), 1835007));
			let mut fTemp804: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp803, 7)) as usize] };
			...
			let mut iTemp809: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp808, 262143)))), 1835007));
			let mut fTemp810: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp809, 7)) as usize] };
			...
			let mut iTemp823: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp822, 262143)))), 1835007));
			let mut fTemp824: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp823, 7), 1835007))) as usize] };
			let mut fTemp825: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp823 as usize] };
			let mut fTemp826: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp823, 1), 1835007))) as usize] } - fTemp825;
			let mut iTemp827: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp825 + fTemp734 * fTemp826 + (fTemp821 - (iTemp822) as F64) * (fTemp824 - (fTemp825 + fTemp734 * (fTemp826 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp823, 8), 1835007))) as usize] } - fTemp824))))} else {1.0 - (fTemp819 + fTemp734 * fTemp820 + (fTemp815 - (iTemp816) as F64) * (fTemp818 - (fTemp819 + fTemp734 * (fTemp820 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp817, 8)) as usize] } - fTemp818)))))} - fTemp813) / (1.0 - fTemp813))) as i32;
			...
			let mut iTemp834: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp833, 262143)))), 1835007));
			let mut fTemp835: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp834, 7)) as usize] };
			...
			let mut iTemp840: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp839, 262143)))), 1835007));
			let mut fTemp841: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp840, 7)) as usize] };
			...
			let mut iTemp854: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp853, 262143)))), 1835007));
			let mut fTemp855: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp854, 7), 1835007))) as usize] };
			let mut fTemp856: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp854 as usize] };
			let mut fTemp857: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp854, 1), 1835007))) as usize] } - fTemp856;
			let mut iTemp858: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp856 + fTemp734 * fTemp857 + (fTemp852 - (iTemp853) as F64) * (fTemp855 - (fTemp856 + fTemp734 * (fTemp857 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp854, 8), 1835007))) as usize] } - fTemp855))))} else {1.0 - (fTemp850 + fTemp734 * fTemp851 + (fTemp846 - (iTemp847) as F64) * (fTemp849 - (fTemp850 + fTemp734 * (fTemp851 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp848, 8)) as usize] } - fTemp849)))))} - fTemp844) / (1.0 - fTemp844))) as i32;
			...
			let mut iTemp865: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp864, 262143)))), 1835007));
			let mut fTemp866: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp865, 7)) as usize] };
			...
			let mut iTemp871: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp870, 262143)))), 1835007));
			let mut fTemp872: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp871, 7)) as usize] };
			...
			let mut iTemp879: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp878, 262143)))), 1835007));
			let mut fTemp880: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp879, 7)) as usize] };
			...
			let mut iTemp885: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp884, 262143)))), 1835007));
			let mut fTemp886: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp885, 7), 1835007))) as usize] };
			let mut fTemp887: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp885 as usize] };
			let mut fTemp888: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp885, 1), 1835007))) as usize] } - fTemp887;
			let mut iTemp889: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp887 + fTemp734 * fTemp888 + (fTemp883 - (iTemp884) as F64) * (fTemp886 - (fTemp887 + fTemp734 * (fTemp888 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp885, 8), 1835007))) as usize] } - fTemp886))))} else {1.0 - (fTemp881 + fTemp734 * fTemp882 + (fTemp877 - (iTemp878) as F64) * (fTemp880 - (fTemp881 + fTemp734 * (fTemp882 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp879, 8)) as usize] } - fTemp880)))))} - fTemp875) / (1.0 - fTemp875))) as i32;
			...
			let mut iTemp896: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp895, 262143)))), 1835007));
			let mut fTemp897: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp896, 7)) as usize] };
			...
			let mut iTemp902: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp901, 262143)))), 1835007));
			let mut fTemp903: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp902, 7)) as usize] };
			...
			let mut fTemp906: F64 = if iTemp725 != 0 {fTemp904 + fTemp734 * fTemp905 + (fTemp900 - (iTemp901) as F64) * (fTemp903 - (fTemp904 + fTemp734 * (fTemp905 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp902, 8)) as usize] } - fTemp903))))} else {1.0 - (fTemp898 + fTemp734 * fTemp899 + (fTemp894 - (iTemp895) as F64) * (fTemp897 - (fTemp898 + fTemp734 * (fTemp899 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp896, 8)) as usize] } - fTemp897)))))};
			let mut fTemp907: F64 = fTemp739 + fTemp893;
			let mut fTemp908: F64 = 262143.0 * (1.0 - fTemp907);
			let mut iTemp909: i32 = (fTemp908) as i32;
			let mut iTemp910: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp909, 262143)))), 1835007));
			let mut fTemp911: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp910, 7)) as usize] };
			...
			let mut iTemp916: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp915, 262143)))), 1835007));
			let mut fTemp917: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp916, 7), 1835007))) as usize] };
			let mut fTemp918: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp916 as usize] };
			let mut fTemp919: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp916, 1), 1835007))) as usize] } - fTemp918;
			let mut iTemp920: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp918 + fTemp734 * fTemp919 + (fTemp914 - (iTemp915) as F64) * (fTemp917 - (fTemp918 + fTemp734 * (fTemp919 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp916, 8), 1835007))) as usize] } - fTemp917))))} else {1.0 - (fTemp912 + fTemp734 * fTemp913 + (fTemp908 - (iTemp909) as F64) * (fTemp911 - (fTemp912 + fTemp734 * (fTemp913 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp910, 8)) as usize] } - fTemp911)))))} - fTemp906) / (1.0 - fTemp906))) as i32;
			...
			let mut iTemp927: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp926, 262143)))), 1835007));
			let mut fTemp928: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp927, 7)) as usize] };
			...
			let mut iTemp933: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp932, 262143)))), 1835007));
			let mut fTemp934: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp933, 7)) as usize] };
			...
			let mut fTemp937: F64 = if iTemp725 != 0 {fTemp935 + fTemp734 * fTemp936 + (fTemp931 - (iTemp932) as F64) * (fTemp934 - (fTemp935 + fTemp734 * (fTemp936 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp933, 8)) as usize] } - fTemp934))))} else {1.0 - (fTemp929 + fTemp734 * fTemp930 + (fTemp925 - (iTemp926) as F64) * (fTemp928 - (fTemp929 + fTemp734 * (fTemp930 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp927, 8)) as usize] } - fTemp928)))))};
			let mut fTemp938: F64 = fTemp739 + fTemp924;
			let mut fTemp939: F64 = 262143.0 * (1.0 - fTemp938);
			let mut iTemp940: i32 = (fTemp939) as i32;
			let mut iTemp941: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp940, 262143)))), 1835007));
			let mut fTemp942: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp941, 7)) as usize] };
			...
			let mut iTemp947: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp946, 262143)))), 1835007));
			let mut fTemp948: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp947, 7), 1835007))) as usize] };
			let mut fTemp949: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp947 as usize] };
			let mut fTemp950: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp947, 1), 1835007))) as usize] } - fTemp949;
			let mut iTemp951: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp949 + fTemp734 * fTemp950 + (fTemp945 - (iTemp946) as F64) * (fTemp948 - (fTemp949 + fTemp734 * (fTemp950 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp947, 8), 1835007))) as usize] } - fTemp948))))} else {1.0 - (fTemp943 + fTemp734 * fTemp944 + (fTemp939 - (iTemp940) as F64) * (fTemp942 - (fTemp943 + fTemp734 * (fTemp944 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp941, 8)) as usize] } - fTemp942)))))} - fTemp937) / (1.0 - fTemp937))) as i32;
			...
			let mut iTemp958: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp957, 262143)))), 1835007));
			let mut fTemp959: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp958, 7)) as usize] };
			...
			let mut iTemp964: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp963, 262143)))), 1835007));
			let mut fTemp965: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp964, 7)) as usize] };
			...
			let mut fTemp968: F64 = if iTemp725 != 0 {fTemp966 + fTemp734 * fTemp967 + (fTemp962 - (iTemp963) as F64) * (fTemp965 - (fTemp966 + fTemp734 * (fTemp967 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp964, 8)) as usize] } - fTemp965))))} else {1.0 - (fTemp960 + fTemp734 * fTemp961 + (fTemp956 - (iTemp957) as F64) * (fTemp959 - (fTemp960 + fTemp734 * (fTemp961 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp958, 8)) as usize] } - fTemp959)))))};
			let mut fTemp969: F64 = fTemp739 + fTemp955;
			let mut fTemp970: F64 = 262143.0 * (1.0 - fTemp969);
			let mut iTemp971: i32 = (fTemp970) as i32;
			let mut iTemp972: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp971, 262143)))), 1835007));
			let mut fTemp973: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp972, 7)) as usize] };
			...
			let mut iTemp978: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp977, 262143)))), 1835007));
			let mut fTemp979: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp978, 7), 1835007))) as usize] };
			let mut fTemp980: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp978 as usize] };
			let mut fTemp981: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp978, 1), 1835007))) as usize] } - fTemp980;
			let mut iTemp982: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp980 + fTemp734 * fTemp981 + (fTemp976 - (iTemp977) as F64) * (fTemp979 - (fTemp980 + fTemp734 * (fTemp981 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp978, 8), 1835007))) as usize] } - fTemp979))))} else {1.0 - (fTemp974 + fTemp734 * fTemp975 + (fTemp970 - (iTemp971) as F64) * (fTemp973 - (fTemp974 + fTemp734 * (fTemp975 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp972, 8)) as usize] } - fTemp973)))))} - fTemp968) / (1.0 - fTemp968))) as i32;
			...
			let mut iTemp989: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp988, 262143)))), 1835007));
			let mut fTemp990: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp989, 7)) as usize] };
			...
			let mut iTemp995: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp994, 262143)))), 1835007));
			let mut fTemp996: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp995, 7)) as usize] };
			...
			let mut fTemp999: F64 = if iTemp725 != 0 {fTemp997 + fTemp734 * fTemp998 + (fTemp993 - (iTemp994) as F64) * (fTemp996 - (fTemp997 + fTemp734 * (fTemp998 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp995, 8)) as usize] } - fTemp996))))} else {1.0 - (fTemp991 + fTemp734 * fTemp992 + (fTemp987 - (iTemp988) as F64) * (fTemp990 - (fTemp991 + fTemp734 * (fTemp992 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp989, 8)) as usize] } - fTemp990)))))};
			...
			let mut iTemp1003: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1002, 262143)))), 1835007));
			let mut fTemp1004: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1003, 7)) as usize] };
			...
			let mut iTemp1009: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1008, 262143)))), 1835007));
			let mut fTemp1010: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1009, 7), 1835007))) as usize] };
			let mut fTemp1011: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1009 as usize] };
			let mut fTemp1012: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1009, 1), 1835007))) as usize] } - fTemp1011;
			let mut iTemp1013: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp1011 + fTemp734 * fTemp1012 + (fTemp1007 - (iTemp1008) as F64) * (fTemp1010 - (fTemp1011 + fTemp734 * (fTemp1012 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1009, 8), 1835007))) as usize] } - fTemp1010))))} else {1.0 - (fTemp1005 + fTemp734 * fTemp1006 + (fTemp1001 - (iTemp1002) as F64) * (fTemp1004 - (fTemp1005 + fTemp734 * (fTemp1006 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1003, 8)) as usize] } - fTemp1004)))))} - fTemp999) / (1.0 - fTemp999))) as i32;
			...
			let mut iTemp1020: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1019, 262143)))), 1835007));
			let mut fTemp1021: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1020, 7)) as usize] };
			...
			let mut iTemp1026: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1025, 262143)))), 1835007));
			let mut fTemp1027: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1026, 7)) as usize] };
			...
			let mut fTemp1030: F64 = if iTemp725 != 0 {fTemp1028 + fTemp734 * fTemp1029 + (fTemp1024 - (iTemp1025) as F64) * (fTemp1027 - (fTemp1028 + fTemp734 * (fTemp1029 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1026, 8)) as usize] } - fTemp1027))))} else {1.0 - (fTemp1022 + fTemp734 * fTemp1023 + (fTemp1018 - (iTemp1019) as F64) * (fTemp1021 - (fTemp1022 + fTemp734 * (fTemp1023 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1020, 8)) as usize] } - fTemp1021)))))};
			...
			let mut iTemp1034: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1033, 262143)))), 1835007));
			let mut fTemp1035: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1034, 7)) as usize] };
			...
			let mut iTemp1040: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1039, 262143)))), 1835007));
			let mut fTemp1041: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1040, 7), 1835007))) as usize] };
			let mut fTemp1042: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1040 as usize] };
			let mut fTemp1043: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1040, 1), 1835007))) as usize] } - fTemp1042;
			let mut iTemp1044: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp1042 + fTemp734 * fTemp1043 + (fTemp1038 - (iTemp1039) as F64) * (fTemp1041 - (fTemp1042 + fTemp734 * (fTemp1043 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1040, 8), 1835007))) as usize] } - fTemp1041))))} else {1.0 - (fTemp1036 + fTemp734 * fTemp1037 + (fTemp1032 - (iTemp1033) as F64) * (fTemp1035 - (fTemp1036 + fTemp734 * (fTemp1037 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1034, 8)) as usize] } - fTemp1035)))))} - fTemp1030) / (1.0 - fTemp1030))) as i32;
			...
			let mut iTemp1051: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1050, 262143)))), 1835007));
			let mut fTemp1052: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1051, 7)) as usize] };
			...
			let mut iTemp1057: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1056, 262143)))), 1835007));
			let mut fTemp1058: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1057, 7)) as usize] };
			...
			let mut fTemp1061: F64 = if iTemp725 != 0 {fTemp1059 + fTemp734 * fTemp1060 + (fTemp1055 - (iTemp1056) as F64) * (fTemp1058 - (fTemp1059 + fTemp734 * (fTemp1060 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1057, 8)) as usize] } - fTemp1058))))} else {1.0 - (fTemp1053 + fTemp734 * fTemp1054 + (fTemp1049 - (iTemp1050) as F64) * (fTemp1052 - (fTemp1053 + fTemp734 * (fTemp1054 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1051, 8)) as usize] } - fTemp1052)))))};
			...
			let mut iTemp1065: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1064, 262143)))), 1835007));
			let mut fTemp1066: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1065, 7)) as usize] };
			...
			let mut iTemp1071: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1070, 262143)))), 1835007));
			let mut fTemp1072: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1071, 7), 1835007))) as usize] };
			let mut fTemp1073: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1071 as usize] };
			let mut fTemp1074: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1071, 1), 1835007))) as usize] } - fTemp1073;
			let mut iTemp1075: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp1073 + fTemp734 * fTemp1074 + (fTemp1069 - (iTemp1070) as F64) * (fTemp1072 - (fTemp1073 + fTemp734 * (fTemp1074 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1071, 8), 1835007))) as usize] } - fTemp1072))))} else {1.0 - (fTemp1067 + fTemp734 * fTemp1068 + (fTemp1063 - (iTemp1064) as F64) * (fTemp1066 - (fTemp1067 + fTemp734 * (fTemp1068 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1065, 8)) as usize] } - fTemp1066)))))} - fTemp1061) / (1.0 - fTemp1061))) as i32;
			...
			let mut iTemp1082: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1081, 262143)))), 1835007));
			let mut fTemp1083: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1082, 7)) as usize] };
			...
			let mut iTemp1088: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1087, 262143)))), 1835007));
			let mut fTemp1089: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1088, 7)) as usize] };
			...
			let mut fTemp1092: F64 = if iTemp725 != 0 {fTemp1090 + fTemp734 * fTemp1091 + (fTemp1086 - (iTemp1087) as F64) * (fTemp1089 - (fTemp1090 + fTemp734 * (fTemp1091 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1088, 8)) as usize] } - fTemp1089))))} else {1.0 - (fTemp1084 + fTemp734 * fTemp1085 + (fTemp1080 - (iTemp1081) as F64) * (fTemp1083 - (fTemp1084 + fTemp734 * (fTemp1085 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1082, 8)) as usize] } - fTemp1083)))))};
			...
			let mut iTemp1096: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1095, 262143)))), 1835007));
			let mut fTemp1097: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1096, 7)) as usize] };
			...
			let mut iTemp1102: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1101, 262143)))), 1835007));
			let mut fTemp1103: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1102, 7), 1835007))) as usize] };
			let mut fTemp1104: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1102 as usize] };
			let mut fTemp1105: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1102, 1), 1835007))) as usize] } - fTemp1104;
			let mut iTemp1106: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp1104 + fTemp734 * fTemp1105 + (fTemp1100 - (iTemp1101) as F64) * (fTemp1103 - (fTemp1104 + fTemp734 * (fTemp1105 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1102, 8), 1835007))) as usize] } - fTemp1103))))} else {1.0 - (fTemp1098 + fTemp734 * fTemp1099 + (fTemp1094 - (iTemp1095) as F64) * (fTemp1097 - (fTemp1098 + fTemp734 * (fTemp1099 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1096, 8)) as usize] } - fTemp1097)))))} - fTemp1092) / (1.0 - fTemp1092))) as i32;
			...
			let mut iTemp1113: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1112, 262143)))), 1835007));
			let mut fTemp1114: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1113, 7)) as usize] };
			...
			let mut iTemp1119: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1118, 262143)))), 1835007));
			let mut fTemp1120: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1119, 7)) as usize] };
			...
			let mut fTemp1123: F64 = if iTemp725 != 0 {fTemp1121 + fTemp734 * fTemp1122 + (fTemp1117 - (iTemp1118) as F64) * (fTemp1120 - (fTemp1121 + fTemp734 * (fTemp1122 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1119, 8)) as usize] } - fTemp1120))))} else {1.0 - (fTemp1115 + fTemp734 * fTemp1116 + (fTemp1111 - (iTemp1112) as F64) * (fTemp1114 - (fTemp1115 + fTemp734 * (fTemp1116 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1113, 8)) as usize] } - fTemp1114)))))};
			...
			let mut iTemp1127: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1126, 262143)))), 1835007));
			let mut fTemp1128: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1127, 7)) as usize] };
			...
			let mut iTemp1133: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1132, 262143)))), 1835007));
			let mut fTemp1134: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1133, 7), 1835007))) as usize] };
			let mut fTemp1135: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1133 as usize] };
			let mut fTemp1136: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1133, 1), 1835007))) as usize] } - fTemp1135;
			let mut iTemp1137: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp1135 + fTemp734 * fTemp1136 + (fTemp1131 - (iTemp1132) as F64) * (fTemp1134 - (fTemp1135 + fTemp734 * (fTemp1136 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1133, 8), 1835007))) as usize] } - fTemp1134))))} else {1.0 - (fTemp1129 + fTemp734 * fTemp1130 + (fTemp1125 - (iTemp1126) as F64) * (fTemp1128 - (fTemp1129 + fTemp734 * (fTemp1130 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1127, 8)) as usize] } - fTemp1128)))))} - fTemp1123) / (1.0 - fTemp1123))) as i32;
			...
			let mut iTemp1144: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1143, 262143)))), 1835007));
			let mut fTemp1145: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1144, 7)) as usize] };
			...
			let mut iTemp1150: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1149, 262143)))), 1835007));
			let mut fTemp1151: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1150, 7)) as usize] };
			...
			let mut iTemp1158: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1157, 262143)))), 1835007));
			let mut fTemp1159: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1158, 7)) as usize] };
			...
			let mut iTemp1164: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1163, 262143)))), 1835007));
			let mut fTemp1165: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1164, 7), 1835007))) as usize] };
			let mut fTemp1166: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1164 as usize] };
			let mut fTemp1167: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1164, 1), 1835007))) as usize] } - fTemp1166;
			let mut iTemp1168: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp1166 + fTemp734 * fTemp1167 + (fTemp1162 - (iTemp1163) as F64) * (fTemp1165 - (fTemp1166 + fTemp734 * (fTemp1167 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1164, 8), 1835007))) as usize] } - fTemp1165))))} else {1.0 - (fTemp1160 + fTemp734 * fTemp1161 + (fTemp1156 - (iTemp1157) as F64) * (fTemp1159 - (fTemp1160 + fTemp734 * (fTemp1161 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1158, 8)) as usize] } - fTemp1159)))))} - fTemp1154) / (1.0 - fTemp1154))) as i32;
			...
			let mut iTemp1175: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1174, 262143)))), 1835007));
			let mut fTemp1176: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1175, 7)) as usize] };
			...
			let mut iTemp1181: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1180, 262143)))), 1835007));
			let mut fTemp1182: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1181, 7)) as usize] };
			...
			let mut fTemp1185: F64 = if iTemp725 != 0 {fTemp1183 + fTemp734 * fTemp1184 + (fTemp1179 - (iTemp1180) as F64) * (fTemp1182 - (fTemp1183 + fTemp734 * (fTemp1184 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1181, 8)) as usize] } - fTemp1182))))} else {1.0 - (fTemp1177 + fTemp734 * fTemp1178 + (fTemp1173 - (iTemp1174) as F64) * (fTemp1176 - (fTemp1177 + fTemp734 * (fTemp1178 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1175, 8)) as usize] } - fTemp1176)))))};
			...
			let mut iTemp1189: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1188, 262143)))), 1835007));
			let mut fTemp1190: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1189, 7)) as usize] };
			...
			let mut iTemp1195: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1194, 262143)))), 1835007));
			let mut fTemp1196: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1195, 7), 1835007))) as usize] };
			let mut fTemp1197: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1195 as usize] };
			let mut fTemp1198: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1195, 1), 1835007))) as usize] } - fTemp1197;
			let mut iTemp1199: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp1197 + fTemp734 * fTemp1198 + (fTemp1193 - (iTemp1194) as F64) * (fTemp1196 - (fTemp1197 + fTemp734 * (fTemp1198 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1195, 8), 1835007))) as usize] } - fTemp1196))))} else {1.0 - (fTemp1191 + fTemp734 * fTemp1192 + (fTemp1187 - (iTemp1188) as F64) * (fTemp1190 - (fTemp1191 + fTemp734 * (fTemp1192 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1189, 8)) as usize] } - fTemp1190)))))} - fTemp1185) / (1.0 - fTemp1185))) as i32;
			...
			let mut iTemp1206: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1205, 262143)))), 1835007));
			let mut fTemp1207: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1206, 7)) as usize] };
			let mut fTemp1208: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1206 as usize] };
			let mut fTemp1209: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1206, 1)) as usize] } - fTemp1208;
			let mut fTemp1210: F64 = 131071.5 * fTemp1202;
			let mut iTemp1211: i32 = (fTemp1210) as i32;
			let mut iTemp1212: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1211, 262143)))), 1835007));
			let mut fTemp1213: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1212, 7)) as usize] };
			let mut fTemp1214: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1212 as usize] };
			let mut fTemp1215: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1212, 1)) as usize] } - fTemp1214;
			let mut fTemp1216: F64 = if iTemp725 != 0 {fTemp1214 + fTemp734 * fTemp1215 + (fTemp1210 - (iTemp1211) as F64) * (fTemp1213 - (fTemp1214 + fTemp734 * (fTemp1215 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1212, 8)) as usize] } - fTemp1213))))} else {1.0 - (fTemp1208 + fTemp734 * fTemp1209 + (fTemp1204 - (iTemp1205) as F64) * (fTemp1207 - (fTemp1208 + fTemp734 * (fTemp1209 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1206, 8)) as usize] } - fTemp1207)))))};
			...
			let mut iTemp1220: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1219, 262143)))), 1835007));
			let mut fTemp1221: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1220, 7)) as usize] };
			let mut fTemp1222: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1220 as usize] };
			let mut fTemp1223: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1220, 1)) as usize] } - fTemp1222;
			let mut fTemp1224: F64 = 262143.0 * fTemp1217;
			let mut iTemp1225: i32 = (fTemp1224) as i32;
			let mut iTemp1226: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1225, 262143)))), 1835007));
			let mut fTemp1227: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1226, 7), 1835007))) as usize] };
			let mut fTemp1228: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1226 as usize] };
			let mut fTemp1229: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1226, 1), 1835007))) as usize] } - fTemp1228;
			let mut iTemp1230: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp1228 + fTemp734 * fTemp1229 + (fTemp1224 - (iTemp1225) as F64) * (fTemp1227 - (fTemp1228 + fTemp734 * (fTemp1229 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1226, 8), 1835007))) as usize] } - fTemp1227))))} else {1.0 - (fTemp1222 + fTemp734 * fTemp1223 + (fTemp1218 - (iTemp1219) as F64) * (fTemp1221 - (fTemp1222 + fTemp734 * (fTemp1223 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1220, 8)) as usize] } - fTemp1221)))))} - fTemp1216) / (1.0 - fTemp1216))) as i32;
			...
			let mut iTemp1237: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1236, 262143)))), 1835007));
			let mut fTemp1238: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1237, 7)) as usize] };
			let mut fTemp1239: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1237 as usize] };
			let mut fTemp1240: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1237, 1)) as usize] } - fTemp1239;
			let mut fTemp1241: F64 = 131071.5 * fTemp1233;
			let mut iTemp1242: i32 = (fTemp1241) as i32;
			let mut iTemp1243: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1242, 262143)))), 1835007));
			let mut fTemp1244: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1243, 7)) as usize] };
			let mut fTemp1245: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1243 as usize] };
			let mut fTemp1246: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1243, 1)) as usize] } - fTemp1245;
			let mut fTemp1247: F64 = if iTemp725 != 0 {fTemp1245 + fTemp734 * fTemp1246 + (fTemp1241 - (iTemp1242) as F64) * (fTemp1244 - (fTemp1245 + fTemp734 * (fTemp1246 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1243, 8)) as usize] } - fTemp1244))))} else {1.0 - (fTemp1239 + fTemp734 * fTemp1240 + (fTemp1235 - (iTemp1236) as F64) * (fTemp1238 - (fTemp1239 + fTemp734 * (fTemp1240 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1237, 8)) as usize] } - fTemp1238)))))};
			...
			let mut iTemp1251: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1250, 262143)))), 1835007));
			let mut fTemp1252: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1251, 7)) as usize] };
			let mut fTemp1253: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1251 as usize] };
			let mut fTemp1254: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1251, 1)) as usize] } - fTemp1253;
			let mut fTemp1255: F64 = 262143.0 * fTemp1248;
			let mut iTemp1256: i32 = (fTemp1255) as i32;
			let mut iTemp1257: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1256, 262143)))), 1835007));
			let mut fTemp1258: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1257, 7), 1835007))) as usize] };
			let mut fTemp1259: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1257 as usize] };
			let mut fTemp1260: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1257, 1), 1835007))) as usize] } - fTemp1259;
			let mut iTemp1261: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp1259 + fTemp734 * fTemp1260 + (fTemp1255 - (iTemp1256) as F64) * (fTemp1258 - (fTemp1259 + fTemp734 * (fTemp1260 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1257, 8), 1835007))) as usize] } - fTemp1258))))} else {1.0 - (fTemp1253 + fTemp734 * fTemp1254 + (fTemp1249 - (iTemp1250) as F64) * (fTemp1252 - (fTemp1253 + fTemp734 * (fTemp1254 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1251, 8)) as usize] } - fTemp1252)))))} - fTemp1247) / (1.0 - fTemp1247))) as i32;
			...
			let mut iTemp1268: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1267, 262143)))), 1835007));
			let mut fTemp1269: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1268, 7)) as usize] };
			let mut fTemp1270: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1268 as usize] };
			let mut fTemp1271: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1268, 1)) as usize] } - fTemp1270;
			let mut fTemp1272: F64 = 131071.5 * fTemp1264;
			let mut iTemp1273: i32 = (fTemp1272) as i32;
			let mut iTemp1274: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1273, 262143)))), 1835007));
			let mut fTemp1275: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1274, 7)) as usize] };
			let mut fTemp1276: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1274 as usize] };
			let mut fTemp1277: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1274, 1)) as usize] } - fTemp1276;
			let mut fTemp1278: F64 = if iTemp725 != 0 {fTemp1276 + fTemp734 * fTemp1277 + (fTemp1272 - (iTemp1273) as F64) * (fTemp1275 - (fTemp1276 + fTemp734 * (fTemp1277 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1274, 8)) as usize] } - fTemp1275))))} else {1.0 - (fTemp1270 + fTemp734 * fTemp1271 + (fTemp1266 - (iTemp1267) as F64) * (fTemp1269 - (fTemp1270 + fTemp734 * (fTemp1271 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1268, 8)) as usize] } - fTemp1269)))))};
			...
			let mut iTemp1282: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1281, 262143)))), 1835007));
			let mut fTemp1283: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1282, 7)) as usize] };
			let mut fTemp1284: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1282 as usize] };
			let mut fTemp1285: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1282, 1)) as usize] } - fTemp1284;
			let mut fTemp1286: F64 = 262143.0 * fTemp1279;
			let mut iTemp1287: i32 = (fTemp1286) as i32;
			let mut iTemp1288: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1287, 262143)))), 1835007));
			let mut fTemp1289: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1288, 7), 1835007))) as usize] };
			let mut fTemp1290: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1288 as usize] };
			let mut fTemp1291: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1288, 1), 1835007))) as usize] } - fTemp1290;
			let mut iTemp1292: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp1290 + fTemp734 * fTemp1291 + (fTemp1286 - (iTemp1287) as F64) * (fTemp1289 - (fTemp1290 + fTemp734 * (fTemp1291 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1288, 8), 1835007))) as usize] } - fTemp1289))))} else {1.0 - (fTemp1284 + fTemp734 * fTemp1285 + (fTemp1280 - (iTemp1281) as F64) * (fTemp1283 - (fTemp1284 + fTemp734 * (fTemp1285 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1282, 8)) as usize] } - fTemp1283)))))} - fTemp1278) / (1.0 - fTemp1278))) as i32;
			...
			let mut iTemp1299: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1298, 262143)))), 1835007));
			let mut fTemp1300: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1299, 7)) as usize] };
			let mut fTemp1301: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1299 as usize] };
			let mut fTemp1302: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1299, 1)) as usize] } - fTemp1301;
			let mut fTemp1303: F64 = 131071.5 * fTemp1295;
			let mut iTemp1304: i32 = (fTemp1303) as i32;
			let mut iTemp1305: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1304, 262143)))), 1835007));
			let mut fTemp1306: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1305, 7)) as usize] };
			let mut fTemp1307: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1305 as usize] };
			let mut fTemp1308: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1305, 1)) as usize] } - fTemp1307;
			let mut fTemp1309: F64 = if iTemp725 != 0 {fTemp1307 + fTemp734 * fTemp1308 + (fTemp1303 - (iTemp1304) as F64) * (fTemp1306 - (fTemp1307 + fTemp734 * (fTemp1308 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1305, 8), 1835007))) as usize] } - fTemp1306))))} else {1.0 - (fTemp1301 + fTemp734 * fTemp1302 + (fTemp1297 - (iTemp1298) as F64) * (fTemp1300 - (fTemp1301 + fTemp734 * (fTemp1302 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1299, 8), 1835007))) as usize] } - fTemp1300)))))};
			...
			let mut iTemp1313: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1312, 262143)))), 1835007));
			let mut fTemp1314: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1313, 7)) as usize] };
			let mut fTemp1315: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1313 as usize] };
			let mut fTemp1316: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1313, 1)) as usize] } - fTemp1315;
			let mut fTemp1317: F64 = 262143.0 * fTemp1310;
			let mut iTemp1318: i32 = (fTemp1317) as i32;
			let mut iTemp1319: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1318, 262143)))), 1835007));
			let mut fTemp1320: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1319, 7), 1835007))) as usize] };
			let mut fTemp1321: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1319 as usize] };
			let mut fTemp1322: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1319, 1), 1835007))) as usize] } - fTemp1321;
			let mut iTemp1323: i32 = (fTemp795 > ((if iTemp725 != 0 {fTemp1321 + fTemp734 * fTemp1322 + (fTemp1317 - (iTemp1318) as F64) * (fTemp1320 - (fTemp1321 + fTemp734 * (fTemp1322 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1319, 8), 1835007))) as usize] } - fTemp1320))))} else {1.0 - (fTemp1315 + fTemp734 * fTemp1316 + (fTemp1311 - (iTemp1312) as F64) * (fTemp1314 - (fTemp1315 + fTemp734 * (fTemp1316 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1313, 8)) as usize] } - fTemp1314)))))} - fTemp1309) / (1.0 - fTemp1309))) as i32;
			let mut fTemp1324: F64 = F64::min(1.0, F64::max(0.0, 0.5 * (if iTemp1323 != 0 {fTemp1296} else {fTemp1294} + if iTemp1323 != 0 {fTemp1293} else {fTemp1296})));
			self.fRec15[0] = fTemp1324;
			let mut fTemp1325: F64 = 262143.0 * (1.0 - fTemp1324);
			let mut iTemp1326: i32 = (fTemp1325) as i32;
			let mut iTemp1327: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1326, 262143)))), 1835007));
			let mut fTemp1328: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1327, 7)) as usize] };
			let mut fTemp1329: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1327 as usize] };
			let mut fTemp1330: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1327, 1)) as usize] } - fTemp1329;
			let mut fTemp1331: F64 = 262143.0 * fTemp1324;
			let mut iTemp1332: i32 = (fTemp1331) as i32;
			let mut iTemp1333: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1332, 262143)))), 1835007));
			let mut fTemp1334: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1333, 7)) as usize] };
			let mut fTemp1335: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1333 as usize] };
			let mut fTemp1336: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1333, 1)) as usize] } - fTemp1335;
			let mut fTemp1337: F64 = if iTemp725 != 0 {fTemp1335 + fTemp734 * fTemp1336 + (fTemp1331 - (iTemp1332) as F64) * (fTemp1334 - (fTemp1335 + fTemp734 * (fTemp1336 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1333, 8), 1835007))) as usize] } - fTemp1334))))} else {1.0 - (fTemp1329 + fTemp734 * fTemp1330 + (fTemp1325 - (iTemp1326) as F64) * (fTemp1328 - (fTemp1329 + fTemp734 * (fTemp1330 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1327, 8), 1835007))) as usize] } - fTemp1328)))))};
			...
			let mut iTemp1341: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1340, 262143)))), 1835007));
			let mut fTemp1342: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1341, 7)) as usize] };
			let mut fTemp1343: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1341 as usize] };
			let mut fTemp1344: F64 = unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1341, 1)) as usize] } - fTemp1343;
			let mut fTemp1345: F64 = 262143.0 * fTemp1338;
			let mut iTemp1346: i32 = (fTemp1345) as i32;
			let mut iTemp1347: i32 = std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp729, i32::wrapping_mul(7, std::cmp::max(0, std::cmp::min(iTemp1346, 262143)))), 1835007));
			let mut fTemp1348: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1347, 7), 1835007))) as usize] };
			let mut fTemp1349: F64 = unsafe { ftbl0LambRs96kSIG0[iTemp1347 as usize] };
			let mut fTemp1350: F64 = unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1347, 1), 1835007))) as usize] } - fTemp1349;
			let mut fTemp1351: F64 = self.fRec16[1] + if ((0.001 * fTemp738) == 0.0) as i32 != 0 {fTemp724} else {fTemp724 * (if iTemp725 != 0 {fTemp1349 + fTemp734 * fTemp1350 + (fTemp1345 - (iTemp1346) as F64) * (fTemp1348 - (fTemp1349 + fTemp734 * (fTemp1350 - (unsafe { ftbl0LambRs96kSIG0[(std::cmp::max(0, std::cmp::min(i32::wrapping_add(iTemp1347, 8), 1835007))) as usize] } - fTemp1348))))} else {1.0 - (fTemp1343 + fTemp734 * fTemp1344 + (fTemp1339 - (iTemp1340) as F64) * (fTemp1342 - (fTemp1343 + fTemp734 * (fTemp1344 - (unsafe { ftbl0LambRs96kSIG0[(i32::wrapping_add(iTemp1341, 8)) as usize] } - fTemp1342)))))} - fTemp1337) / (1.0 - fTemp1337)};
			self.fRec16[0] = if iTemp737 != 0 {F64::min(fTemp1351, self.fRec16[1])} else {F64::max(fTemp1351, self.fRec16[1])};
			self.fVec62[(self.IOTA0 & 16383) as usize] = F64::powf(1e+01, 0.05 * self.fRec16[0]);
			let mut fTemp1352: F64 = self.fVec62[((i32::wrapping_sub(self.IOTA0, iSlow74)) & 16383) as usize];
			*output1 = 0.5 * fTemp2 * self.fVec3[((i32::wrapping_sub(self.IOTA0, iSlow75)) & 32767) as usize] + self.fRec14[0] * fTemp4 * self.fVec4[((i32::wrapping_sub(self.IOTA0, iSlow75)) & 32767) as usize] * fTemp1352;
